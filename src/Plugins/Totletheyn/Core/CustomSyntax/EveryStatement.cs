...
[CustomCodeFragment]
public sealed class EveryStatement(string interval, string aliasName, FunctionDefinition callable)
    : CodeNode
{
    public static bool Validate(string code, int position)
    {
        return Parser.Validate(code, "every", position);
    }

    public static CodeNode Parse(ParseInfo state, ref int position)
    {
        if (!Parser.Validate(state.Code, "every", ref position))
            return null;
        ...
        return new EveryStatement(interval, aliasName, callable);
    }
