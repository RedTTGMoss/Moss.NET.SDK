...
using Totletheyn.Core.RSS.Parser;
...
public static class FeedReader
{
    ...
    public static HtmlFeedLink GetAbsoluteFeedUrl(string pageUrl, HtmlFeedLink feedLink)
    {
        ...
        if (Uri.TryCreate(tmpUrl, UriKind.RelativeOrAbsolute, out var finalUri))
        {
            ...
            if (Uri.TryCreate(pageUrl + '/' + tmpUrl.TrimStart('/'), UriKind.Absolute, out finalUri))
                return new HtmlFeedLink(feedLink.Title.HtmlDecode(), finalUri.ToString(), feedLink.FeedType);
        }
    ...
    {
        var feed = ReadAsync(url).GetAwaiter().GetResult();
        return feed;
    }
    ...
    {
        var feedContent = await Helpers.DownloadBytesAsync(GetAbsoluteUrl(url)).ConfigureAwait(false);
