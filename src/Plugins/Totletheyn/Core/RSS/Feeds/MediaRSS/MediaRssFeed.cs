...
public class MediaRssFeed : BaseFeed
{
        ...
        : base(feedXml, channel)
    {
        ...
        foreach (var item in items) Items.Add(new MediaRssFeedItem(item));
    }
    ...
    private void ParseDates(string language, string publishingDate, string lastBuildDate)
    {
        ...
        if (parseLocalizedPublishingDate) PublishingDate = Helpers.TryParseDateTime(PublishingDateString, culture);

        if (parseLocalizedLastBuildDate) LastBuildDate = Helpers.TryParseDateTime(LastBuildDateString, culture);
    }
}