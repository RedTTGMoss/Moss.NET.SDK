...
{
    private static readonly WeakReference<AssemblyBuilder> dynamicAssembly = new(null);
    private static readonly WeakReference<ModuleBuilder> dynamicModule = new(null);
    ...
    public static bool Is<T>(this JSValue self)
    {
...
#endif
        {
            ...
            {
                return self.Is(JSValueType.Integer) ||
                       (self.Is(JSValueType.Double) && self._dValue == (long)self._dValue);
            }
            ...
            {
                return (self.Is(JSValueType.Integer) && self._iValue >= 0) ||
                       (self.Is(JSValueType.Double) && self._dValue == (ulong)self._dValue);
            }
            default:
            {
                return false;
    ...
    public static T As<T>(this JSValue self)
    {
        ...
        {
            case TypeCode.Double:
                return GetDefinedOr(self, (T)(object)double.NaN);
        }

        return GetDefinedOr(self, default(T));
    }
    ...
    public static T AsImplementationOf<T>(this JSValue self, Context context) where T : class
    {
        ...
        lock (dynamicAssembly)
        {
            ...
            {
                assemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("<nil.js>DynamicAssembly"),
                    AssemblyBuilderAccess.RunAndCollect);
                dynamicAssembly.SetTarget(assemblyBuilder);

                var m = assemblyBuilder.GetDynamicModule("InterfaceImplementations") ??
                        assemblyBuilder.DefineDynamicModule("InterfaceImplementations");
        ...
        if (type == null)
        {
            ...
            type = module.DefineType(typename, TypeAttributes.Class | TypeAttributes.Public | TypeAttributes.Sealed,
                baseType, interfaces);
            ...
            var delegatesField = type.DefineField("_delegates", typeof(Delegate[]),
                FieldAttributes.Private | FieldAttributes.Static);
            var delegates = new Delegate[methods.Length];

            var jsValParameter = Expression.Parameter(typeof(JSValue), "jsval");

            var getPropertyMethod = typeof(JSValue).GetMethod(nameof(JSValue.GetProperty),
                BindingFlags.NonPublic | BindingFlags.Instance, null,
                [typeof(JSValue), typeof(bool), typeof(PropertyScope)], null);
                ...
                var functionTree = Expression.Convert(
                    Expression.Property(
                        Expression.Call(
                            ...
                            getPropertyMethod,
                            Expression.Constant(
                                (JSValue)(methods[i].GetCustomAttribute<JavaScriptNameAttribute>()?.Name ??
                                          method.Name)),
            ...
            type.CreateType().GetField(delegatesField.Name, BindingFlags.Static | BindingFlags.NonPublic)
                .SetValue(null, delegates);
        }
