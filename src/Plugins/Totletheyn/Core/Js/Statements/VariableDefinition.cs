...
public sealed class VariableDefinition : CodeNode
{
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index, bool forForLoop)
    {
        ...
        var s = position;
        while (state.Code[position] != ';' && state.Code[position] != '}' &&
               !Tools.IsLineTerminator(state.Code[position]))
        {
            Tools.SkipSpaces(state.Code, ref position);

            if (state.Code[position] != '[' && state.Code[position] != '{' &&
                !Parser.ValidateName(state.Code, position, state.Strict))
            {
                if (Parser.ValidateName(state.Code, ref position, false, true, state.Strict))
                    ExceptionHelper.ThrowSyntaxError('\"' +
                                                     Tools.Unescape(state.Code.Substring(s, position - s),
                                                         state.Strict) +
                                                     "\" is a reserved word, but used as a variable. " +
                                                     CodeCoordinates.FromTextPosition(state.Code, s, position - s));
                ExceptionHelper.ThrowSyntaxError("Invalid variable definition at " +
                                                 CodeCoordinates.FromTextPosition(state.Code, s, position - s));
            }
            ...
            if (expression is VariableReference)
            {
                ...
                if (state.Strict)
                    if (name == "arguments" || name == "eval")
                        ExceptionHelper.ThrowSyntaxError(
                            "Varible name cannot be \"arguments\" or \"eval\" in strict mode", state.Code, s,
                            position - s);
            ...
            for (var j = 0; j < state.Variables.Count - i + skiped; j++)
                if (state.Variables[j].name == names[i] && state.Variables[j].definitionScopeLevel >= level)
                {
                    if (state.Variables[j].lexicalScope || mode > VariableKind.FunctionScope)
                        ExceptionHelper.ThrowSyntaxError(string.Format(Strings.IdentifierAlreadyDeclared, names[i]),
                            state.Code, index);
    ...
    public override JSValue Evaluate(Context context)
    {
        ...
        if (context._executionMode == ExecutionMode.Regular)
            for (var v = 0; v < _variables.Length; v++)
                if (context._executionMode == ExecutionMode.Regular && Kind > VariableKind.FunctionScope &&
                    _variables[v].lexicalScope)
                {
                    var f = context.DefineVariable(_variables[v].name);
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        {
            Parser.Build(ref _initializers[i], message != null ? 2 : expressionDepth, variables, codeContext, message,
                stats, opts);
    ...
    {
        for (var i = 0; i < _initializers.Length; i++) _initializers[i].Decompose(ref _initializers[i]);
    }
