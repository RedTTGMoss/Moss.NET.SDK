...
public sealed class DoWhile : CodeNode
{
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        if (!Parser.Validate(state.Code, "while", ref i))
            ExceptionHelper.Throw(new SyntaxError("Expected \"while\" at + " +
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        try
        {
            if (_allowRemove
                ...
                && (_condition is Constant || (_condition as Expression).ContextIndependent))
            {
    ...
    {
        return "do" +
               (_body is CodeBlock ? _body + " " : Environment.NewLine + "  " + _body + ";" + Environment.NewLine) +
               "while (" + _condition + ")";
    }
}