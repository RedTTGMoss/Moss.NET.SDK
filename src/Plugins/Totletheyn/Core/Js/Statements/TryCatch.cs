...
public sealed class TryCatch : CodeNode
{
    ...
    public CodeNode CatchBody => catchBody;
    public CodeNode FinalBody => finallyBody;
    public string ExceptionVariableName => catchVariableDesc.name;
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        if (state.Code[i] != '{')
            ExceptionHelper.Throw(new SyntaxError("Invalid try statement definition at " +
        ...
        if (Parser.Validate(state.Code, "catch (", ref i) || Parser.Validate(state.Code, "catch(", ref i))
        {
            ...
            if (!Parser.ValidateName(state.Code, ref i, state.Strict))
                ExceptionHelper.Throw(new SyntaxError("Catch block must contain variable name " +
                                                      CodeCoordinates.FromTextPosition(state.Code, i, 0)));
            ...
            if (state.Strict)
                if (exptn == "arguments" || exptn == "eval")
                    ExceptionHelper.Throw(new SyntaxError(
                        "Varible name can not be \"arguments\" or \"eval\" in strict mode at " +
            ...
            while (Tools.IsWhiteSpace(state.Code[i]))
                i++;
            if (state.Code[i] != '{')
                ExceptionHelper.Throw(new SyntaxError("Invalid catch block statement definition at " +
                                                      CodeCoordinates.FromTextPosition(state.Code, i, 0)));
        ...
        if (Parser.Validate(state.Code, "finally", i) && Parser.IsIdentifierTerminator(state.Code[i + 7]))
        {
            ...
            while (Tools.IsWhiteSpace(state.Code[i]))
                i++;
            if (state.Code[i] != '{')
                ExceptionHelper.Throw(new SyntaxError("Invalid finally block statement definition at " +
                                                      CodeCoordinates.FromTextPosition(state.Code, i, 0)));
            f = CodeBlock.Parse(state, ref i);
        }

        if (cb == null && f == null)
            ExceptionHelper.ThrowSyntaxError("try block must contain 'catch' or/and 'finally' block", state.Code,
                index);
...
#endif
        {
            cvar = e is JSException
                ? (e as JSException).Error.CloneImpl(false)
                : context.GlobalContext.ProxyValue(new NativeError(e.Message) { exception = e });
        }
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        if (catchBody != null)
        {
            ...
            Parser.Build(ref catchBody, expressionDepth, variables, codeContext | CodeContext.Conditional, message,
                stats, opts);
            if (oldVarDesc != null)
        ...
        if (body == null || body is Empty)
        {
            if (message != null)
                message(MessageLevel.Warning, Position, Length,
                    "Empty (or reduced to empty) try" + (catchBody != null ? "..catch" : "") +
                    (finallyBody != null ? "..finally" : "") + " block. Maybe, something missing.");
        ...
        if (_catch && (catchBody == null || catchBody is Empty))
            if (message != null)
                message(MessageLevel.Warning, catchPosition, (catchBody ?? this).Length,
                    "Empty (or reduced to empty) catch block. Do not ignore exceptions.");

        if (finallyPosition != 0 && (finallyBody == null || finallyBody is Empty))
            if (message != null)
                message(MessageLevel.Warning, catchPosition, (catchBody ?? this).Length,
                    "Empty (or reduced to empty) finally block.");
    ...
    {
        var sbody = (body as CodeBlock as object ?? "{" + Environment.NewLine + " " + body + Environment.NewLine + "}")
            .ToString();
        var fbody = finallyBody == null
            ? ""
            : (finallyBody as CodeBlock as object ??
               "{" + Environment.NewLine + " " + finallyBody + Environment.NewLine + "}").ToString();
        var cbody = catchBody == null
            ? ""
            : (catchBody as CodeBlock as object ??
               "{" + Environment.NewLine + " " + catchBody + Environment.NewLine + "}").ToString();
        return "try" +
               sbody +
               (catchBody != null
                   ? Environment.NewLine +
