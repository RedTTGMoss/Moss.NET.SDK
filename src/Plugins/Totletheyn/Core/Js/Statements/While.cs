...
public sealed class While : CodeNode
{
    ...
    public ICollection<string> Labels => new ReadOnlyCollection<string>(_labels);
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        var ccs = state.ContiniesCount;
        var cbs = state.BreaksCount;
        var body = Parser.Parse(state, ref i, 0);
        if (body is FunctionDefinition)
        {
            if (state.Message != null)
                state.Message(MessageLevel.CriticalWarning, body.Position, body.Length,
                    Strings.DoNotDeclareFunctionInNestedBlocks);
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        Parser.Build(ref _body, expressionDepth, variables, codeContext | CodeContext.Conditional | CodeContext.InLoop,
            message, stats, opts);
        ...
        {
            if (message != null)
                message(MessageLevel.Warning, _condition.Position, 2,
                    "Useless conversion. Remove double negation in condition");
            _condition = (_condition as Expression)._left;
        }

        if (_allowRemove && (_condition is Constant ||
                             (_condition is Expression && (_condition as Expression).ContextIndependent)))
        {
            Eliminated = true;
