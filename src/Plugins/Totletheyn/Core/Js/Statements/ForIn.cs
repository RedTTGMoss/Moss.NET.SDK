...
public sealed class ForIn : CodeNode
{
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        try
        {
            ...
            if (variableDef == null)
            {
                ...
                variableName = Tools.Unescape(state.Code.Substring(variableNameStart, i - variableNameStart),
                    state.Strict);
    ...
    public override JSValue Evaluate(Context context)
    {
        ...
        if (context._executionMode >= ExecutionMode.Resume) suspendData = context.SuspendData[this] as SuspendData;
        ...
        if (suspendData == null || suspendData.variable == null)
        {
            ...
            if (varialeDefStat != null)
            {
                ...
                variable =
                    (varialeDefStat._initializers[0]._left ?? varialeDefStat._initializers[0])
                    .EvaluateForWrite(context);
            }
        ...
        var index = 0;
        var processedKeys = (suspendData != null ? suspendData.processedKeys : null) ??
                            new HashSet<string>(StringComparer.Ordinal);
        while (source != null)
        {
            var keys = (suspendData != null ? suspendData.keys : null) ??
                       source.GetEnumerator(false, EnumerationMode.RequireValues);
            ...
            if (source == JSValue.@null || !source.Defined ||
                (source._valueType >= JSValueType.Object && source._oValue == null))
                break;
        }
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        if (message != null
            && (_source is ObjectDefinition
                ...
                || _source is Constant))
            message(MessageLevel.Recomendation, Position, Length,
                "for..in with constant source. This reduce performance. Rewrite without using for..in.");
    ...
    {
        return "for (" + _variable + " in " + _source + ")" + (_body is CodeBlock ? "" : Environment.NewLine + "  ") +
               _body;
    }
