...
public sealed class CodeBlock : CodeNode
{
    ...
    {
        var position = index;
        var sroot = position == 0 && state.AllowDirectives;
        if (!sroot)
        {
        ...
        using (state.WithCodeContext())
        {
            ...
            try
            {
                    ...
                    do
                    {
                        ...
                        if (Parser.ValidateValue(state.Code, ref position))
                        {
                            ...
                            if (Parser.ValidateString(state.Code, ref t, true))
                            {
                                ...
                                if (str == "use strict") state.CodeContext |= CodeContext.Strict;
                ...
                for (var j = body.Count; j-- > 0;)
                    (body[j] as Constant).value._oValue =
                        Tools.Unescape((body[j] as Constant).value._oValue.ToString(), state.Strict);
                    ...
                    if (t == null)
                    {
                        if (position < state.Code.Length)
                        {
                            if (sroot && state.Code[position] == '}')
                                ExceptionHelper.Throw(new SyntaxError("Unexpected symbol \"}\" at " +
                                                                      CodeCoordinates.FromTextPosition(state.Code,
                                                                          position, 0)));
            ...
            {
                if (oldVariablesCount != state.Variables.Count) variables = extractVariables(state, oldVariablesCount);
                ...
                if (state.Variables[i].definitionScopeLevel == state.LexicalScopeLevel)
                {
                    ...
                    if (declaredVariables != null)
                    {
                        if (declaredVariables.Contains(variables[targetIndex].name) &&
                            variables[targetIndex].lexicalScope)
                            ExceptionHelper.ThrowSyntaxError(
                                "Variable \"" + variables[targetIndex].name + "\" already has been defined", state.Code,
                                i);
    ...
    protected internal override CodeNode[] GetChildrenImpl()
    {
        ...
        if (_variables != null)
            res.AddRange(from v in _variables
                where v.initializer != null && (!(v.initializer is FunctionDefinition) ||
                                                (v.initializer as FunctionDefinition)._body != this)
        ...
        if (_variables != null && _variables.Length != 0)
        {
            for (var i = 0; i < _variables.Length; i++)
            {
                ...
                if (variables.TryGetValue(_variables[i].name, out desc) &&
                    desc.definitionScopeLevel < _variables[i].definitionScopeLevel)
                {
            ...
            if (_lines[i] != null)
            {
                ...
                else
                {
                    if (unreachable && message != null)
                        message(MessageLevel.CriticalWarning, _lines[i].Position, _lines[i].Length,
                    ...
                    Parser.Build(
                        ref cn,
                        (codeContext & CodeContext.InEval) != 0 ? 2 : Math.Max(1, expressionDepth),
                        variables, codeContext | (_strict ? CodeContext.Strict : CodeContext.None),
                        message,
                        stats,
                        opts | (!unreachable && i == lastRealExp
                            ? Options.SuppressUselessExpressionsElimination |
    ...
    internal void Optimize(ref CodeBlock self, FunctionDefinition owner, InternalCompilerMessageCallback message,
        ...
        Options opts, FunctionInfo stats)
    {
        ...
        if (_variables != null)
            for (var i = 0; i < _variables.Length; i++)
                if (_variables[i].initializer != null)
                {
                    ...
                    _variables[i].initializer = cn as Expressions.Expression ?? _variables[i].initializer;
                }
        ...
        for (var i = 0; i < _lines.Length; i++)
        {
            ...
            cn.Optimize(
                ...
                owner,
                message,
                opts | (i == _lines.Length - 1
                    ? Options.SuppressUselessExpressionsElimination | Options.SuppressUselessStatementsElimination
                    : Options.None),
                stats);
            _lines[i] = cn;
        }
        ...
        if (_lines.Length == 1 && _suppressScopeIsolation == SuppressScopeIsolationMode.Suppress &&
            _variables.Length == 0) _this = _lines[0];
    }
        ...
        Dictionary<string, VariableDescriptor> transferedVariables, int scopeBias)
    {
        if (_variables != null)
        {
            ...
            if (_variables.Length != 0 && !functionInfo.WithLexicalEnvironment)
            {
                for (var i = 0; i < _variables.Length; i++)
                {
                    ...
                    if (!transferedVariables.TryGetValue(_variables[i].name, out desc) ||
                        _variables[i].initializer != null)
        ...
        {
            for (var i = 0; i < _lines.Length; i++) _lines[i].RebuildScope(functionInfo, null, scopeBias);
        }
        else
        {
            bool needRerun;
...
#if !PORTABLE
    internal override Expression TryCompile(bool selfCompile, bool forAssign, Type expectedType,
