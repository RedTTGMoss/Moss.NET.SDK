...
public sealed class Throw : CodeNode
{
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        while (i < state.Code.Length && Tools.IsWhiteSpace(state.Code[i]) && !Tools.IsLineTerminator(state.Code[i]))
            i++;
        var b = state.Code[i] == ';' || Tools.IsLineTerminator(state.Code[i])
            ? null
            : (Expression)Parser.Parse(state, ref i, CodeFragmentType.Expression);
        if (b is Empty)
            ExceptionHelper.Throw(new SyntaxError("Can't throw result of EmptyStatement " +
                                                  CodeCoordinates.FromTextPosition(state.Code, i - 1, 0)));
        var pos = index;
        index = i;
        return new Throw(b)
        {
