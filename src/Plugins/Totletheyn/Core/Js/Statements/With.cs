...
public sealed class With : CodeNode
{
    ...
    public override JSValue Evaluate(Context context)
    {
        ...
        action = c =>
        {
            ...
            try
            {
                ...
                if (c._executionMode == ExecutionMode.Suspend) c.SuspendData[this] = action;
            }
            finally
            {
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        Parser.Build(ref _scope, expressionDepth + 1, variables, codeContext | CodeContext.InExpression, message, stats,
            opts);
        Parser.Build(ref _body, expressionDepth, new Dictionary<string, VariableDescriptor>(),
            codeContext | CodeContext.InWith, message, stats, opts);
        return false;
    }
