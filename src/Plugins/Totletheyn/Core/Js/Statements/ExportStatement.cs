...
{
    private readonly List<KeyValuePair<string, Expression>> _map = new();
        ...
        else
        {
            using (state.WithCodeContext(CodeContext.InExport))
            {
                ...
                var definition =
                    ...
                    ?? FunctionDefinition.Parse(state, ref index, FunctionKind.Function)
                    ?? FunctionDefinition.Parse(state, ref index, FunctionKind.AsyncFunction);
    ...
    private static void parseExportMap(ExportStatement export, ParseInfo state, ref int index)
    {
        ...
        while (state.Code[index] != '}')
        {
            ...
            var alias = parseAlias(state.Code, ref index) ?? name;
    ...
    public override JSValue Evaluate(Context context)
    {
        ...
        else if (_internalDefinition != null)
        {
            ...
            if (_internalDefinition is VariableDefinition variableDef)
            {
                for (var i = 0; i < variableDef._variables.Length; i++)
                    context._module.Exports[variableDef._variables[i].name] =
                        variableDef._variables[i].references[0].Evaluate(context);
            }
            else
            {
        ...
        {
            for (var i = 0; i < _map.Count; i++) context._module.Exports[_map[i].Key] = _map[i].Value.Evaluate(context);
        }
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        if (_internalDefinition != null)
            Parser.Build(ref _internalDefinition, expressionDepth, variables, codeContext, message, stats,
                opts | Options.SuppressUselessStatementsElimination);
        else
            for (var i = 0; i < _map.Count; i++)
            {
                var v = _map[i].Value;
