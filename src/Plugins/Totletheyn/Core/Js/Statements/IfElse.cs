...
public sealed class IfElse : CodeNode
{
    ...
    public CodeNode Then => then;
    public CodeNode Else => @else;
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        if (Parser.Validate(state.Code, "else", ref i))
        {
            ...
            {
                if (state.Message != null)
                    state.Message(MessageLevel.CriticalWarning, elseBody.Position, elseBody.Length,
                        Strings.DoNotDeclareFunctionInNestedBlocks);
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        Parser.Build(ref @else, expressionDepth, variables, codeContext | CodeContext.Conditional, message, stats,
            opts);

        if ((opts & Options.SuppressUselessStatementsElimination) == 0 && condition is ConvertToBoolean)
        {
            if (message != null)
                message(MessageLevel.Warning, condition.Position, 2,
    ...
    {
        var rp = Environment.NewLine;
        var rs = Environment.NewLine + "  ";
