...
public sealed class SparseArray<TValue> : IList<TValue>, IDictionary<int, TValue>, ICloneable
{
    ...
    [CLSCompliant(false)] public uint Length { get; private set; }
    ...
    public IEnumerable<KeyValuePair<int, TValue>> ForwardOrder
    {
        get
        {
            ...
            if (index < Length)
                yield return new KeyValuePair<int, TValue>((int)Length - 1, default);
        }
    }
                ...
                if (_navigationData[i].Length != 0)
                    for (var j = 0; j < _used[i]; j++)
                        yield return new KeyValuePair<int, TValue>(
                            _navigationData[i].Length is 0 ? virtualSegmentIndex + j : (int)_navigationData[i][j].Index,
                            _values[i][j]!);
                else
                    for (var j = 0; j < _values[i].Length; j++)
                    {
                        if (virtualSegmentIndex + j >= Length)
                            break;
    ...
    private struct SegmentNavigationItem
    {
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => _oneNext | (int)(((_index >> (SegmentNextAdditionOffset + SegmentNextAdditionSize)) &
                                     SegmentNextAdditionMask) << 16);
            set
            {
                _oneNext = (ushort)value;
                _index &= ~(SegmentNextAdditionMask << (SegmentNextAdditionOffset + SegmentNextAdditionSize));
                _index |= (uint)((value >> 16) & SegmentNextAdditionMask) <<
                          (SegmentNextAdditionOffset + SegmentNextAdditionSize);
            }
        }

        public override string ToString()
        {
            return SegmentIndex + "[" + SegmentZeroNext + ";" + SegmentOneNext + "]";
        }
    }
    ...
    private void resizeL0(int newRealSegmentsCount)
    {
        ...
        Array.Resize(ref _navigationData,
            newRealSegmentsCount <= 1 ? 1 : 1 << NumberUtils.IntLog(newRealSegmentsCount));
    ...
    private enum FindNearestMode
    {
        NotLess,
        NotMore
    }

    private (int realSegmentIndex, int virtualSegmentIndex, int realItemIndex) findNearest(long index,
        FindNearestMode mode)
    {
        ...
        if (mode is FindNearestMode.NotLess)
        {
            ...
            while (true)
            {
                ...
                if (nextSegmentIndex == 0)
                {
                    ...
                    if (segmentNavItem.SegmentOneNext == 0)
                    {
                        if (alterI == -1)
                            return (-1, -1, -1);
        ...
        while (true)
        {
            ...
            if (segmentNavItem.SegmentIndex == virtualSegmentIndex)
            {
                ...
                if (_navigationData[realSegmentIndex].Length == 0)
                    return (realSegmentIndex, segmentNavItem.SegmentIndex,
                        Math.Min(_values[realSegmentIndex].Length - 1, (int)(index & (SegmentSize - 1))));
            ...
            if (segmentNavItem.SegmentIndex > virtualSegmentIndex) return (-1, -1, -1);
    ...
    private int segmentIndexRecalc(int segmentIndex, bool forRead)
    {
        ...
        if (navi.Length == 0)
        {
            ...
            _segmentsNavigationData = [new SegmentNavigationItem { SegmentIndex = segmentIndex }];
            _segmentsCount = 1;
            _navigationData = [[]];
            _values = [[]];
            _used = [0];
    ...
    public TValue this[int index]
    {
        ...
        {
            var isDefault =
                value is null; // структуры мы будем записывать, иначе пришлось бы вызывать тяжелые операции сравнения.
    ...
    {
        var coord = findNearest(Length - 1, FindNearestMode.NotMore);
        var externalIndex = getExternalIndex(coord);
        Length = (uint)externalIndex + 1;
    }
    ...
    public void CopyTo(TValue[] array, int arrayIndex)
    {
        ...
        for (var i = Math.Min(Length, int.MaxValue) + arrayIndex; i-- > arrayIndex;) array[i] = default;
