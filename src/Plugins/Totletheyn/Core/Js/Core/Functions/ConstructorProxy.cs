...
internal class ConstructorProxy : Function
{
        ...
        : base(context)
    {
        ...
        if (_staticProxy._hostedType.ContainsGenericParameters)
            ExceptionHelper.ThrowTypeError(_staticProxy._hostedType.Name +
                                           " can't be created because it's generic type.");
#endif
        ...
        if (withNewOnly && withoutNewOnly)
            ExceptionHelper.Throw(new InvalidOperationException("Unacceptably use of " +
                                                                typeof(RequireNewKeywordAttribute).Name + " and " +
                                                                typeof(DisallowNewKeywordAttribute).Name +
                                                                " for same type."));
        ...
        for (var i = 0; i < ctors.Length; i++)
        {
            ...
            if (ctors[i].GetParameters().Any(x =>
                    x.ParameterType.IsPointer || x.IsOut || x.IsIn || x.IsRetval || x.ParameterType.IsByRef))
                continue;

            if (!ctors[i].IsDefined(typeof(HiddenAttribute), false) ||
                ctors[i].IsDefined(typeof(ForceUseAttribute), true))
            {
    ...
    protected internal override JSValue GetProperty(JSValue key, bool forWrite, PropertyScope memberScope)
    {
        if (memberScope < PropertyScope.Super && key._valueType != JSValueType.Symbol)
        {
            ...
            if (keyString ==
                "prototype") // Все прокси-прототипы read-only и non-configurable. Это и оптимизация, и устранение необходимости навешивания атрибутов
                return prototype;
            ...
            if (forWrite || (keyString != "toString" && keyString != "constructor"))
            {
                ...
                if (memberScope == PropertyScope.Own && (res._valueType != JSValueType.Property ||
    ...
    {
        if (!construct && _staticProxy._hostedType == typeof(Date)) return new Date().ToString();
...
&& _staticProxy._hostedType.GetTypeInfo().IsValueType)
        ...
        if (construct)
        {
            ...
            else
            {
                ...
                if (obj.GetType() == typeof(Date))
                    ...
                    objc._valueType = (JSValueType)Math.Max((int)objc._valueType, (int)res._valueType);
        ...
        else
        {
            ...
            if (res == null) res = wrapObject(targetObject, obj);
        }
    ...
    private ObjectWrapper wrapObject(JSValue targetObject, object obj)
    {
        var objc = (targetObject as ObjectWrapper ?? ConstructObject()) as ObjectWrapper;
        objc.instance = obj;
        objc._attributes |= _staticProxy._hostedType.GetTypeInfo().IsDefined(typeof(ImmutableAttribute), false)
            ? JSValueAttributesInternal.Immutable
            : JSValueAttributesInternal.None;
        return objc;
    }

    protected internal override JSValue ConstructObject()
    {
        return new ObjectWrapper(null, Context.GlobalContext.GetPrototype(_staticProxy._hostedType));
    }

    private MethodProxy findConstructor(Arguments arguments, out object[] args)
    {
        args = null;
        var paramsCount = arguments == null ? 0 : arguments._iValue;
        for (var pass = 0; pass < passesCount; pass++)
        for (var i = 0; i < _constructors.Length; i++)
        {
            if (_constructors[i]._parameters.Length == 1 &&
                _constructors[i]._parameters[0].ParameterType == typeof(Arguments))
            {
                args = [arguments];
                return _constructors[i];
            }

            if (_constructors[i]._parameters.Length == paramsCount
                || (_constructors[i]._parameters.Length >= paramsCount
                    && _constructors[i]._parameters.Count(x => !x.IsOptional) <= paramsCount))
            {
                if (paramsCount == 0)
                {
