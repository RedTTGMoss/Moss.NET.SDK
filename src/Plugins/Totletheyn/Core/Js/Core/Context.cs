...
{
    [ThreadStatic] private static List<Context> _currentContextStack;
    ...
    public static GlobalContext DefaultGlobalContext { get; } = new() { _strict = true };
    ...
    public GlobalContext GlobalContext
    {
        get
        {
            ...
            var result = iter as GlobalContext;
    ...
    protected internal virtual JSValue GetVariable(string name, bool forWrite)
    {
        ...
        var fromProto = _variables == null || (!_variables.TryGetValue(name, out res) && _parent != null);
        if (fromProto)
    ...
    /// <param name="suppressScopeCreation">
    ///     If true, scope will not be created. All variables, which will be defined via let,
    ///     const or class will not be destructed after evalution
    /// </param>
    /// <returns>Result of last evaluated operation</returns>
    public JSValue Eval(string sourceCode, JSValue thisBind, bool suppressScopeCreation = false)
    {
        ...
        CodeNode cb = body;
        Parser.Build(ref cb, 0, variables, (_strict ? CodeContext.Strict : CodeContext.None) | CodeContext.InEval, null,
            stats, Options.None);
        ...
        body.Optimize(ref cb, null, null,
            Options.SuppressUselessExpressionsElimination | Options.SuppressConstantPropogation, stats);
        ...
        try
        {
            var context = suppressScopeCreation || (!stats.WithLexicalEnvironment && !body._strict && !_strict)
                ? this
                : new Context(this, false, _owner)
                {
                ...
                for (var i = 0; i < body._variables.Length; i++)
                    if (!body._variables[i].lexicalScope)
                    {
                        ...
                        while (cc._parent._parent != null
                               && (cc._variables == null ||
                                   !cc._variables.TryGetValue(body._variables[i].name, out variable)))
                            cc = cc._parent;
            ...
            catch (JSException e)
            {
                if ((e.SourceCode == null || e.CodeCoordinates == null) && e.ExceptionMaker != null)
                {
                    ...
                    e.CodeCoordinates = CodeCoordinates.FromTextPosition(sourceCode, e.ExceptionMaker.Position,
                        e.ExceptionMaker.Length);
                }
