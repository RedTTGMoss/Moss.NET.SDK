...
public class JSObject : JSValue
{
    ...
    public sealed override JSObject __proto__
    {
        ...
        get
        {
            ...
            if (_valueType >= JSValueType.Object && _oValue != this)
            {
                ...
                if (oValueAsJs != null) return oValueAsJs.__proto__;
            }
    ...
    {
#if DEBUG
        Debug.Assert(_oValue == this || !(_oValue is JSValue), "Look out!");
#endif
        JSValue res = null;
        JSObject proto = null;
        var fromProto = false;
        ...
        else
        {
            ...
            if (fromProto)
            {
                ...
                if (propertyScope == PropertyScope.Own && ((res._attributes & JSValueAttributesInternal.Field) == 0 ||
                ...
                if ((res._attributes & JSValueAttributesInternal.SystemObject) != 0 || fromProto)
                        ...
                        && (res._valueType != JSValueType.Property || propertyScope == PropertyScope.Own))
                    {
                        ...
                        if ((res._attributes & JSValueAttributesInternal.Field) == 0)
                            res._attributes &= ~(JSValueAttributesInternal.DoNotEnumerate |
    ...
    protected internal override void SetProperty(JSValue key, JSValue value, PropertyScope propertyScope,
        bool throwOnError)
    {
        JSValue field;
        if (_valueType >= JSValueType.Object && _oValue != this)
        {
            if (_oValue == null)
                ExceptionHelper.Throw(new TypeError("Cannot get property \"" + key + "\" of \"null\""));
        ...
        if (field._valueType == JSValueType.Property)
        {
            ...
            if (setter != null)
                ...
                ExceptionHelper.Throw(new TypeError("Cannot assign value to readonly property \"" + key + "\""));
        }
        ...
        EnumerationMode enumeratorMode, PropertyScope propertyScope = PropertyScope.Common)
    {
        ...
        if (_objectPrototype != null && propertyScope is not PropertyScope.Own)
            for (var e = _objectPrototype.GetEnumerator(hideNonEnum, EnumerationMode.RequireValues,
            ...
            foreach (var item in members)
            {
                ...
                if (writable.Exists && (get.Exists || set.Exists))
                    ExceptionHelper.Throw(
                        new TypeError("Property can not have getter or setter and writable attribute."));
    ...
    public static JSValue defineProperties(Arguments args)
    {
        ...
        if (args[0]._oValue == null)
            ExceptionHelper.Throw(new TypeError("Cannot define properties of null."));
    ...
    public static JSValue defineProperty(Arguments args)
    {
        ...
        var memberName = args[1].ToString();
    ...
    private static JSObject definePropertyImpl(JSObject target, JSObject desc, string memberName)
    {
        ...
        if ((obj._attributes & JSValueAttributesInternal.Argument) != 0 && (set.Exists || get.Exists))
        {
            ...
            if (target is Arguments &&
                int.TryParse(memberName, NumberStyles.Integer, CultureInfo.InvariantCulture, out ti) && ti >= 0 &&
                ti < 16)
        ...
        if ((obj._attributes & JSValueAttributesInternal.SystemObject) != 0)
            ExceptionHelper.Throw(new TypeError("Cannot define property \"" + memberName + "\". Object immutable."));
            ...
            if (memberName == "length")
                try
                {
                    if (value.Exists)
                    {
                        ...
                        if (!(target as Array).SetLength(nlen))
                            ExceptionHelper.Throw(
                                new TypeError("Unable to reduce length because Exists not configurable elements"));
        ...
        if (!config)
        {
            if (enumerable.Exists &&
                (obj._attributes & JSValueAttributesInternal.DoNotEnumerate) != 0 == (bool)enumerable)
                ExceptionHelper.Throw(
            ...
            if (obj._valueType == JSValueType.Property && (obj._attributes & JSValueAttributesInternal.Field) == 0
                                                       ...
                                                       && (((obj._oValue as PropertyPair).setter != null &&
                                                            (obj._oValue as PropertyPair).setter._oValue != set._oValue)
                                                           || ((obj._oValue as PropertyPair).setter == null &&
                                                               set.Defined)))
            ...
            if (obj._valueType == JSValueType.Property && (obj._attributes & JSValueAttributesInternal.Field) == 0
                                                       ...
                                                       && (((obj._oValue as PropertyPair).getter != null &&
                                                            (obj._oValue as PropertyPair).getter._oValue != get._oValue)
                                                           || ((obj._oValue as PropertyPair).getter == null &&
                                                               get.Defined)))
            ...
            if (!config
                ...
                && !((StrictEqual.Check(obj, value) &&
                      ((obj._valueType == JSValueType.Undefined && value._valueType == JSValueType.Undefined) ||
                       ...
                       1.0 / Tools.JSObjectToDouble(obj) == 1.0 / Tools.JSObjectToDouble(value)))
                     || (obj._valueType == JSValueType.Double && value._valueType == JSValueType.Double &&
                         double.IsNaN(obj._dValue) && double.IsNaN(value._dValue))))
    ...
    public static JSValue freeze(Arguments args)
    {
        ...
        for (var e = obj.GetEnumerator(false, EnumerationMode.RequireValuesForWrite); e.MoveNext();)
        {
            ...
            if ((value._attributes & JSValueAttributesInternal.SystemObject) == 0)
                value._attributes |= JSValueAttributesInternal.NonConfigurable | JSValueAttributesInternal.ReadOnly |
                                     JSValueAttributesInternal.DoNotDelete;
        }
    ...
    public static JSValue preventExtensions(Arguments args)
    {
        ...
        if (args[0]._oValue == null)
            ExceptionHelper.Throw(new TypeError("Cannot prevent extensions for null"));
        var obj = args[0].Value as JSObject ?? args[0]._oValue as JSObject;
        obj._attributes |= JSValueAttributesInternal.Immutable;
        return obj;
    }
    ...
    public static Array getOwnPropertySymbols(JSObject obj)
    {
        return new Array(obj?._symbols?.Keys ?? new Symbol[0]);
    }
