...
using Totletheyn.Core.Js.Core.Functions;
...
using JSBool = Totletheyn.Core.Js.BaseLibrary.Boolean;
using Math = System.Math;
using String = Totletheyn.Core.Js.BaseLibrary.String;
...
public static class Tools
{
    ...
    private static readonly Type[] intTypeWithinArray = [typeof(int)];
    ...
    private static readonly IntStringCacheItem[]
        intStringCache = new IntStringCacheItem[cacheSize]; // Обрати внимание на константы внизу
        ...
        do
        {
            ...
            switch (arg._valueType)
            {
                ...
                case JSValueType.String:
                {
                    ...
                    if (ParseJsNumber(s, ref ix, out x, 0,
                            ParseNumberOptions.AllowFloat | ParseNumberOptions.AllowAutoRadix) && ix < s.Length)
                        return double.NaN;
    ...
    /// </summary>
    /// <param name="arg">JSObject, значение которого нужно преобразовать.</param>
    /// <param name="alternateInfinity">
    ///     Если истина, для значений +Infinity и -Infinity будут возвращены значения int.MaxValue
    ///     и int.MinValue соответственно.
    /// </param>
    /// <returns>Целочисленное значение, представленное в объекте arg.</returns>
#if !NET40
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        switch (value._valueType)
        {
            ...
            case JSValueType.Double:
            {
                ...
                if (double.IsInfinity(value._dValue))
                    return alternateInfinity
                        ? double.IsPositiveInfinity(value._dValue) ? int.MaxValue : int.MinValue
                        : 0;
            ...
            case JSValueType.String:
            {
                ...
                if (!ParseJsNumber(s, ref ix, out x, 0,
                        ParseNumberOptions.AllowAutoRadix | ParseNumberOptions.AllowFloat) || ix < s.Length)
                    return 0;
    ...
    internal static JSValue JSObjectToNumber(JSValue arg, JSValue result)
    {
        ...
        switch (arg._valueType)
        {
            ...
            case JSValueType.String:
            {
                ...
                if (!ParseJsNumber(s, ref ix, out x,
    ...
    {
        return ConvertJStoObj(jsobj, targetType, true);
    }
    ...
    internal static object ConvertJStoObj(JSValue jsobj, Type targetType, bool hightLoyalty)
    {
        ...
        switch (jsobj._valueType)
        {
            ...
            case JSValueType.String:
            {
...
#else
                        if (Enum.TryParse(targetType, jsobj.Value.ToString(), out var result))
                            return result;
#endif
                    }

                    if (targetType == typeof(Guid)) return Guid.Parse(jsobj.Value.ToString());
                ...
                if (targetType == typeof(DateTime))
                    return DateTime.TryParse(jsobj.Value.ToString(), out var dateTime)
                        ? dateTime
                        : new Date(new Arguments { jsobj.Value.ToString() }).ToDateTime();
...
#endif
                if (iEnumerableInterface != null) elementType = iEnumerableInterface.GetGenericArguments()[0];
            }
            ...
            if (value is Array array)
                    ...
                    || elementType == typeof(object))
                {
                    ...
                    if (targetType.IsAssignableFrom(typeof(List<>).MakeGenericType(elementType)))
                        return convertCollection(value as Array, elementType,
                            typeof(List<>).MakeGenericType(elementType), hightLoyalty);
                }
        }
        ...
        if (hightLoyalty)
        {
                ...
                && !typeof(JSValue).IsAssignableFrom(targetType))
            {
                ...
                var targetInstance = (Context.CurrentGlobalContext.GetConstructor(targetType) as Function)
            ...
            if (targetType.GetTypeInfo().IsValueType) return Activator.CreateInstance(targetType);
        }

        return null;
    }

    private static IList convertCollection(Array array, Type elementType, Type collectionType, bool hightLoyalty)
    {
        if (array == null)
            return null;

        var len = (int)array._data.Length;
        var result = (IList)Activator.CreateInstance(collectionType, len);
        ...
        for (var j = 0; j < len; j++)
        {
            ...
            if (!hightLoyalty && value == null &&
                (elementType.GetTypeInfo().IsValueType || (!temp.IsNull && !temp.IsUndefined())))
                return null;
    ...
    public static bool ParseJsNumber(string code, ref int index, out double value, int radix,
        ParseNumberOptions options)
    {
            ...
            && i + 1 < code.Length)
        {
            if (NumberUtils.IsDigit(code[i + 1]))
            {
                ...
                while (i + 1 < code.Length && code[i + 1] == '0') i++;
        ...
        while (i < code.Length)
        {
            ...
            if (degSign >= radix || (NumChars[degSign] != code[i] && NumChars[degSign] + ('a' - 'A') != code[i])) break;
            ...
            if (code[i] == '\\' && i + 1 < code.Length)
            {
                ...
                switch (code[i])
                {
                            ...
                            else
                            {
                                if (processRegexComp)
                                    ...
                                    ExceptionHelper.Throw(
                                        new SyntaxError("Invalid escape sequence '\\" + code[i] + c + "'"));
                            }
                        }
                    ...
                    default:
                    {
                        ...
                        {
                            if (strict && (code[i] != '0' ||
                                           (code.Length > i + 1 && code[i + 1] >= '0' && code[i + 1] <= '7')))
                                ExceptionHelper.Throw(
                                    new SyntaxError("Octal literals are not allowed in strict mode."));
...
#endif
    public static string UnescapeNextChar(string code, int index, out int processedChars, bool strict,
        bool processUnknown, bool processRegexComp)
    {
        return UnescapeNextChar(code, index, out processedChars, strict, processUnknown, processRegexComp, true);
    }

    public static string UnescapeNextChar(string code, int index, out int processedChars, bool strict,
        bool processUnknown, bool processRegexComp, bool fullUnicode)
    {
        processedChars = 0;
    ...
    {
        return i >= 0 && i + 1 < str.Length && str[i] >= '\uD800' && str[i] <= '\uDBFF' && str[i + 1] >= '\uDC00' &&
               str[i + 1] <= '\uDFFF';
    }
    ...
    {
        if (src._oValue is Array array)
            return array._data.Length;

        var length =
            src.GetProperty("length", true,
                PropertyScope.Common); // тут же проверка на null/undefined с падением если надо
    ...
    internal static Array arraylikeToArray(JSValue src, bool evalProps, bool clone, bool reassignLen, long length)
    {
        var temp = new Array();
        var goDeep = true;
        for (; goDeep;)
        {
            ...
            if (srca != null)
            {
                ...
                foreach (var element in srca._data.ForwardOrder)
                {
                    ...
                    if (!goDeep && Math.Abs(prew - element.Key) > 1) goDeep = true;

                    if (evalProps && value._valueType == JSValueType.Property)
                        value = (value._oValue as PropertyPair).getter == null
                            ? JSValue.undefined
                            : (value._oValue as PropertyPair).getter.Call(src, null).CloneImpl(false);
                    else if (clone)
            ...
            else
            {
                ...
                foreach (var index in EnumerateArraylike(length, src))
                {
                    ...
                    if (!goDeep && Math.Abs(prew - index.Key) > 1) goDeep = true;
    ...
    internal static IEnumerable<KeyValuePair<uint, JSValue>> EnumerateArraylike(long length, JSValue src)
    {
        ...
        if (src._valueType == JSValueType.Object && src.Value is Array)
            foreach (var item in (src.Value as Array)._data.ForwardOrder)
            {
                if (item.Key - prev > 1)
                {
        ...
        Type delegateType)
    {
        ...
        var handlerArgumentsParameters = new ParameterExpression[prms.Length + (thisParameter != null ? 1 : 0)];
        {
            ...
            for (var prmI = 0; i < handlerArgumentsParameters.Length; i++, prmI++)
                handlerArgumentsParameters[i] =
                    System.Linq.Expressions.Expression.Parameter(prms[prmI].ParameterType, prms[prmI].Name);
        }
        ...
        if (prms.Length != 0)
        {
            expressions.Add(System.Linq.Expressions.Expression.Assign(argumentsParameter,
                System.Linq.Expressions.Expression.New(typeof(Arguments))));
            for (var i = 0; i < prms.Length; i++)
            {
                System.Linq.Expressions.Expression argument =
                    handlerArgumentsParameters[i + (thisParameter != null ? 1 : 0)];
#if (PORTABLE || NETCORE)
                ...
                expressions.Add(System.Linq.Expressions.Expression.Call(
                    argumentsParameter,
                    typeof(Arguments).GetRuntimeMethod("Add", [typeof(JSValue)]),
        ...
        var argsTypes = thisParameter is not null
            ? new[] { typeof(JSValue), typeof(Arguments) }
            : new[] { typeof(Arguments) };
        var callMethod = typeof(Function).GetRuntimeMethod(nameof(Function.Call), argsTypes);
        var callTree = thisParameter is null
            ? System.Linq.Expressions.Expression.Call(functionGetter, callMethod, argumentsParameter)
            : System.Linq.Expressions.Expression.Call(functionGetter, callMethod, thisParameter, argumentsParameter);
        ...
        {
            var asMethod = typeof(JSValueExtensions).GetRuntimeMethods().First(x => x.Name == "As")
                .MakeGenericMethod(method.ReturnParameter.ParameterType);
            ...
            while (pos < f.Length && hasNext)
            {
                ...
                switch (c)
                {
                    ...
                    case 'd':
                        ...
                        else if (!ParseJsNumber((o ?? "null").ToString(), out d, 0))
                            d = double.NaN;
                        ...
                        var dstr = NumberUtils.DoubleToString(Math.Abs(d));
                        if (d < 0)
                            s.Append('-');
                        if (dstr.Length < para)
