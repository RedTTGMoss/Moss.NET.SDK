...
namespace Totletheyn.Core.Js.Core.JIT;

#if !NETCORE
internal static class JITHelpers
{
    public static readonly FieldInfo _items =
        typeof(List<CodeNode>).GetField("_items", BindingFlags.Instance | BindingFlags.NonPublic);
    ...
    public static readonly Expression UndefinedConstant = Expression.Field(null,
        typeof(JSValue).GetField("undefined", BindingFlags.Static | BindingFlags.NonPublic));

    public static readonly Expression NotExistsConstant = Expression.Field(null,
        typeof(JSValue).GetField("notExists", BindingFlags.Static | BindingFlags.NonPublic));

    public static readonly MethodInfo JSObjectToBooleanMethod;

    public static readonly MethodInfo JSObjectToInt32Method =
        typeof(Tools).GetMethod("JSObjectToInt32", [typeof(JSValue)]);

    internal static readonly MethodInfo EvaluateForWriteMethod =
        typeof(CodeNode).GetMethod("EvaluateForWrite", [typeof(Context)]);
...
#endif
        {
            ...
            default:
            {
                dest._oValue = new ObjectWrapper(source,
                    source is null ? null : Context.CurrentGlobalContext.GetPrototype(source.GetType()));
