    ...
    where TKey : IComparable<TKey>
{
    ...
    internal IEnumerator<Node> enumerateReversed(Node node)
    {
        ...
        {
            var sstate = state;
            var stack = new Node[node.height];
            var step = new int[node.height];
            var sindex = -1;
            stack[++sindex] = node;
            while (sindex >= 0)
            {
        ...
        if (c != null)
            do
            {
                var cmp = comparer != null
                    ? comparer.Compare(keyValue, c.key)
                    : (keyValue as IComparable).CompareTo(c.key);
                if (cmp <= 0)
                {
    ...
    {
        private readonly BinaryTree<TKey, TValue> owner;
        ...
        {
            foreach (var kvp in owner) yield return kvp.Value;
        }
    ...
    private sealed class _Keys : ICollection<TKey>
    {
        ...
        {
            foreach (var kvp in owner) yield return kvp.Key;
        }
    ...
    internal sealed class Node
    {
        ...
        public void Balance(ref Node _this)
        {
            ...
            var delta = lessH - greaterH;
            if (delta > 1)
            {
                var llessH = less.less != null ? less.less.height : 0;
                var lgreaterH = less.greater != null ? less.greater.height : 0;
                if (llessH < lgreaterH)
                {
            ...
            {
                var mlessH = greater.less != null ? greater.less.height : 0;
                var ggreaterH = greater.greater != null ? greater.greater.height : 0;
                if (mlessH > ggreaterH)
                {
