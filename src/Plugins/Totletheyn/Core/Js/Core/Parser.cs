...
public static class Parser
{
    ...
    [
        ...
        new() // Общий
        {
            ...
            new Rule("import (", ExpressionTree.Parse),
        ...
        // 1
        new() // Начало выражения
        {
            new Rule("[", ExpressionTree.Parse),
            new Rule("{", ExpressionTree.Parse),
            new Rule("await", AwaitExpression.Parse),
            new Rule("function", ExpressionTree.Parse),
            new Rule("async function", ExpressionTree.Parse),
            ...
            new Rule("void", ExpressionTree.Parse),
            new Rule("yield", Yield.Parse),
            new Rule("import (", Import.Parse),
            new Rule(ValidateName, ExpressionTree.Parse),
            new Rule(ValidateValue, ExpressionTree.Parse)
        },
        // 2
        new() // Сущности внутри выражения
        {
    ...
    public static bool Validate(string code, string pattern, ref int index)
    {
        ...
        var result = IsIdentifierTerminator(pattern[pattern.Length - 1]) || j >= code.Length ||
                     IsIdentifierTerminator(code[j]);
        if (result)
            index = j;
    ...
    public static bool ValidateName(string code)
    {
    ...
    [CLSCompliant(false)]
    public static bool ValidateName(string code, int index, bool strict)
    {
        return ValidateName(code, ref index, true, true, strict);
    }
    ...
    public static bool ValidateName(string code, ref int index, bool checkReservedWords, bool allowEscape, bool strict)
    {
        ...
        {
            var name = code.Substring(index, j - index);
            if (allowEscape)
            {
                var i = 0;
    ...
    {
        var fictive = 0.0;
        return Tools.ParseJsNumber(code, ref index, out fictive, 0,
            ParseNumberOptions.AllowFloat | ParseNumberOptions.AllowAutoRadix);
    }
    ...
    public static bool ValidateRegex(string code, ref int index, bool throwError)
    {
        ...
        if (code[j] == '/')
        {
            ...
            while (j < code.Length && (escape || code[j] != '/'))
            {
                ...
                {
                    if (code[j] == '[') escape = true;
                    if (code[j] == ']') escape = false;
                }
    ...
    internal static string RemoveComments(string code, int startPosition)
    {
        ...
        {
            while (index < code.Length && Tools.IsWhiteSpace(code[index])) index++;
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index, CodeFragmentType ruleSet, bool throwError)
    {
        ...
        var sindex = index;
        if (state.Code[index] == ',' || state.Code[index] == ';') return null;

        if (index >= state.Code.Length)
            return null;

        for (var i = 0; i < rules[(int)ruleSet].Count; i++)
    ...
    internal static void Build<T>(ref T self, int expressionDepth, Dictionary<string, VariableDescriptor> variables,
    ...
    internal static void Build(ref Expression s, int expressionDepth, Dictionary<string, VariableDescriptor> variables,
        CodeContext codeContext, InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        CodeNode t = s;
    ...
    public static void DefineCustomCodeFragment(Type type)
    {
        ...
        if (attributes.Length == 0)
            throw new ArgumentException(
                "type must be marked with attribute \"" + typeof(CustomCodeFragment).Name + "\"");
        ...
        var validateMethod = type.GetRuntimeMethod("Validate", [typeof(string), typeof(int)]);
        if (validateMethod == null || validateMethod.ReturnType != typeof(bool))
            throw new ArgumentException(
                "type must contain static method \"Validate\" which get String and Int32 and returns Boolean");

        var parserMethod = type.GetRuntimeMethod("Parse", [typeof(ParseInfo), typeof(int).MakeByRefType()]);
        if (parserMethod == null || parserMethod.ReturnType != typeof(CodeNode))
            throw new ArgumentException("type must contain static method \"Parse\" which get " +
                                        typeof(ParseInfo).Name + " and Int32 by reference and returns " +
                                        typeof(CodeNode).Name);
