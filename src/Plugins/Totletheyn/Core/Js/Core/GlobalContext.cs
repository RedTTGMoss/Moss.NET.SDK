...
using Math = Totletheyn.Core.Js.BaseLibrary.Math;
...
public sealed class GlobalContext : Context
{
    ...
    internal void ResetContext()
    {
        ...
        try
        {
            ...
            _stringPrototype = GetPrototype(typeof(String));
            _numberPrototype = GetPrototype(typeof(Number));
            _booleanPrototype = GetPrototype(typeof(Boolean));
    ...
    {
        if (!_proxies.TryGetValue(type, out var constructor))
            lock (_proxies)
            {
                ...
                else
                {
                    var indexerSupport = IndexersSupport == IndexersSupport.ForceEnable
                                         || (IndexersSupport == IndexersSupport.WithAttributeOnly && type.GetTypeInfo()
                                             .IsDefined(typeof(UseIndexersAttribute), false));
                    ...
                    dynamicProxy._attributes |= JSValueAttributesInternal.DoNotDelete |
    ...
    public JSValue ProxyValue(object value)
    {
...
#endif
        {
            ...
            default:
            {
                ...
                if ((MarshalingOptions & MarshalinOptions.DictionaryAsObject) != 0
                    && value is IEnumerable
                    && (value is IDictionary || value.GetType().GetInterfaces().Any(x =>
                        x.IsConstructedGenericType && x.GetGenericTypeDefinition() == typeof(IDictionary<,>))))
                {
                    var dictionaryInterface = value.GetType().GetInterfaces().FirstOrDefault(x =>
                        x.IsConstructedGenericType && x.GetGenericTypeDefinition() == typeof(IDictionary<,>));
                ...
                if (value is Task)
                {
                    ...
                    if (Tools.IsTaskOfT(value.GetType()))
                        result = new Task<JSValue>(() =>
                        {
                            try
                            {
                                return ProxyValue(value.GetType().GetMethod("get_Result", Type.EmptyTypes)
                                    .Invoke(value, null));
                            }
                ...
                if (value is IEnumerable && NativeReadOnlyListCtors.IsReadOnlyList(value))
                {
        ...
        if (forWrite && GlobalObjectsAssignMode != GlobalObjectsAssignMode.Allow)
            switch (GlobalObjectsAssignMode)
            {
                ...
                case GlobalObjectsAssignMode.Deny:
                    return new JSValue
                        { _attributes = JSValueAttributesInternal.ReadOnly | JSValueAttributesInternal.SystemObject };

                case GlobalObjectsAssignMode.DenyThrowException:
                    ExceptionHelper.Throw(
                        new ReferenceError("Invalid attempt to assign a value to a variable in the GlobalContext"));
                    break;
            }
