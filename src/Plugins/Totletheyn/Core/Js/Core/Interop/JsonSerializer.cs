...
public class JsonSerializer
{
    ...
    public virtual string Serialize(string key, object value, Function replacer, HashSet<string> keys, string space,
        HashSet<JSValue> processed)
    {
        if (!CanSerialize(value))
            throw new ArgumentException("Cannot serialize value with type \"" + value.GetType().FullName + "\" as \"" +
                                        TargetType.FullName + "\"");
...
#if NETSTANDARD1_3
        var result =
 resultContainer ?? TargetType.GetTypeInfo().DeclaredConstructors.Where(x => x.IsPublic).First(x=>x.GetParameters().Length == 0).Invoke(new object[0]);
#else
    ...
    protected virtual void WriteValue(StringBuilder result, string key, object value, Function replacer,
        HashSet<string> keys, string space, HashSet<JSValue> processed)
    {
        if (value == null)
        {
            result.Append("null");
