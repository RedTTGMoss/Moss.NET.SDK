...
public sealed class NativeList : CustomType, IIterable
{
    ...
    protected internal override JSValue GetProperty(JSValue key, bool forWrite, PropertyScope memberScope)
    {
        if (memberScope < PropertyScope.Super && key._valueType != JSValueType.Symbol)
        {
            ...
            switch (tname._valueType)
            {
                ...
                case JSValueType.String:
                {
                    ...
                    if (str.Length > 0)
                    {
                        ...
                        {
                            var si = 0;
                            if (Tools.ParseJsNumber(tname._oValue.ToString(), ref si, out var dindex)
            ...
            if (isIndex && index >= 0 && index < _data.Count) return new Element(this, index);
        }
    ...
    public Array toJSON()
    {
        ...
        for (var i = 0; i < _data.Count; i++) result[i] = context.ProxyValue(_data[i]);
        ...
        if (propertyScope is PropertyScope.Own or PropertyScope.Common)
            for (var i = 0; i < _data.Count; i++)
                yield return new KeyValuePair<string, JSValue>(Tools.Int32ToString(i),
                    (int)enumerationMode > 0 ? new Element(this, i) : null);
...
#endif
                    {
                        ...
                        case TypeCode.DateTime:
                        {
                            ...
                            base.Assign(new ObjectWrapper(new Date(dateTime),
                                Context.CurrentGlobalContext.GetPrototype(typeof(Date))));
                            break;
                        }
...
#if (PORTABLE || NETCORE)
                                _oValue =
 new MethodProxy(context, ((Delegate)value).GetMethodInfo(), ((Delegate)value).Target);
#else
