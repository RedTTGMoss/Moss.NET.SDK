...
public sealed class UntypedDictionaryWrapper : IDictionary<object, object>
{
    ...
    {
        _target.Add(item.Key, item.Value);
    }
    ...
    {
        return _target.Contains(item.Key) && _target[item.Key].Equals(item.Value);
    }

    public bool ContainsKey(object key)
    {
        return _target.Contains(key);
    }
        ...
        EnumerationMode enumeratorMode, PropertyScope propertyScope = PropertyScope.Common)
    {
        if (propertyScope is PropertyScope.Own or PropertyScope.Common)
        {
            if (enumeratorMode == EnumerationMode.KeysOnly)
                return (_target as IDictionary<string, object>).Keys
                    .Select(x => new KeyValuePair<string, JSValue>(x, null)).GetEnumerator();

            if (enumeratorMode == EnumerationMode.RequireValues) return enumItems(false);
