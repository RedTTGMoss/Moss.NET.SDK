...
#if (PORTABLE || NETCORE)
        _instanceCtor =
 _hostedType.GetTypeInfo().DeclaredConstructors.Where(x => x.IsPublic).FirstOrDefault(x => x.GetParameters().Length == 0 && !x.IsStatic);
#else
        _instanceCtor = _hostedType.GetConstructor(
            BindingFlags.Public | BindingFlags.Instance | BindingFlags.FlattenHierarchy, null, Type.EmptyTypes, null);
#endif
    }
                ...
                try
                {
                    ...
                    if (_instanceCtor != null)
                    {
                        ...
                        else if (typeof(JSObject).IsAssignableFrom(_hostedType))
                        {
                            ...
                            _valueType = (JSValueType)Math.Max((int)JSValueType.Object,
                                (int)_prototypeInstance._valueType);
                        }
                        else
                        {
            ...
            for (var i = 0; i < members.Length; i++)
            {
                ...
                if (property != null)
                {
                    if ((property.GetSetMethod(true) ?? property.GetGetMethod(true)).IsStatic !=
                        !(IsInstancePrototype ^ instanceAttribute))
                        continue;
                    ...
                    try
                    {
                        ...
                        while (parentProperty != null
                               ...
                               && ((property.GetGetMethod() ?? property.GetSetMethod()).Attributes &
                                   MethodAttributes.NewSlot) == 0)
                        {
                            property = parentProperty;
#if (PORTABLE || NETCORE)
                            parentProperty =
 property.DeclaringType.GetTypeInfo().BaseType?.GetRuntimeProperty(property.Name);
#else
                            parentProperty = property.DeclaringType.GetTypeInfo().BaseType
                ...
                if (member is EventInfo
                    && (!(member as EventInfo).GetAddMethod(true).IsPublic ||
                        (member as EventInfo).GetAddMethod(true).IsStatic != !IsInstancePrototype))
                    continue;

                if (member is FieldInfo fieldInfo &&
                    (!fieldInfo.IsPublic || fieldInfo.IsStatic != !IsInstancePrototype))
                    continue;
#if (PORTABLE || NETCORE)
                ...
                if (method != null)
                {
                    if (method.IsStatic != !(IsInstancePrototype ^ instanceAttribute))
                        continue;
                    if (!method.IsPublic && !(method.DeclaringType == typeof(Enum) && method.Name == "GetValue"))
                        continue;
                    ...
                    if (method.DeclaringType == typeof(Enum) && method.Name == "GetValue")
                    {
                        tempMembers.Add("valueOf", new[] { member });
                        continue;
                    }
                    ...
                    if (method.IsVirtual && (method.Attributes & MethodAttributes.NewSlot) == 0)
                    {
                        ...
                        while (parentMethod != null && parentMethod.DeclaringType != typeof(object) &&
                               (method.Attributes & MethodAttributes.NewSlot) == 0)
                        {
                            method = parentMethod;
#if NETCORE
                            parentMethod =
 method.DeclaringType.GetTypeInfo().BaseType?.GetMethod(method.Name, parameterTypes);
#else
                            parentMethod = method.DeclaringType.BaseType?.GetMethod(method.Name,
                ...
                {
                    var nameOverrideAttribute =
                        member.GetCustomAttributes(typeof(JavaScriptNameAttribute), false).ToArray();
                    membername = (nameOverrideAttribute[0] as JavaScriptNameAttribute).Name;
                }
                else
                {
                    membername = membername[0] == '.' ? membername :
                        membername.Contains(".") ? membername.Substring(membername.LastIndexOf('.') + 1) : membername;
    ...
    protected internal override JSValue GetProperty(JSValue key, bool forWrite, PropertyScope memberScope)
    {
        ...
        if (_fields.TryGetValue(name, out var r) && r.Exists)
        {
    ...
    internal JSValue proxyMember(IList<MemberInfo> m)
    {
        ...
        if (m.Count > 1)
        {
            ...
            for (var i = 0; i < m.Count; i++) cache[i] = new MethodProxy(_context, m[i] as MethodBase);
        ...
        else
        {
...
#endif
            {
                case MemberTypes.Method:
                {
                    ...
                    r._attributes &= ~(JSValueAttributesInternal.ReadOnly | JSValueAttributesInternal.DoNotDelete |
                        ...
                        r = new JSValue
                        {
                            ...
                            (
                                new ExternalFunction((thisBind, a) =>
                                    _context.ProxyValue(field.GetValue(field.IsStatic ? null : thisBind.Value))),
                                !m[0].IsDefined(typeof(ReadOnlyAttribute), false)
                                    ? new ExternalFunction((thisBind, a) =>
                                    {
...
#else
                            pinfo.CanRead && pinfo.GetGetMethod(false) != null
                                ? new MethodProxy(_context, pinfo.GetGetMethod(false))
                                : null,
                            pinfo.CanWrite && pinfo.GetSetMethod(false) != null &&
                            !pinfo.IsDefined(typeof(ReadOnlyAttribute), false)
                                ? new MethodProxy(_context, pinfo.GetSetMethod(false))
                                : null
#endif
                        )
                    };
...
new MethodProxy(_context, pinfo.AddMethod)
        ...
        EnumerationMode enumerationMode, PropertyScope propertyScope = PropertyScope.Common)
    {
        ...
        if (propertyScope is not PropertyScope.Own && __proto__ is not null)
            for (var e = __proto__.GetEnumerator(hideNonEnumerable, enumerationMode,
                     PropertyScopeForProto(propertyScope));
                 e.MoveNext();)
