...
public static class NumberUtils
{
    ...
    private static int _CachedDoubleStringsIndex;
    ...
    /// <param name="c"></param>
    /// <remarks>
    ///     Использовать вместо этой функции char.IsDigit не получится. Версия char учитывает региональные особенности,
    ///     что не нужно
    /// </remarks>
    /// <returns></returns>
            ...
            if (IsDigit(source[pos]))
            {
                ...
                if (d != 0 || digitsCount != 0)
                {
                    if (digitsCount < _MaxSafeDigitsInTwoLongs)
                    {
                        ...
                        digits[0] <<= 4;
                        digits[0] = (ulong)((long)digits[0] | d);
                        digitsCount++;
                    }
                }
                else
    ...
    public static int TryParse(string source, int start, out double value)
    {
        ...
        if (position < source.Length && (source[position] == 'e' || source[position] == 'E'))
        {
            ...
            var td = 0;
            var eDigits = 0;
            var eSign = source[position] == '+' || source[position] == '-' ? 44 - source[position++] : 1;
            ...
            while (intDigitsCount < _MaxSafeDigitsInTwoLongs && eDeg != 0)
            {
                ...
                else
                {
                    if (fracDigitsCount > 0)
                    {
                        ...
                        var shift = fracDigitsCount % 16 * 4;
    ...
    {
        if (((ulong)value & 0xffff_ffff_0000_0000ul) != 0) return IntLog((uint)(value >> 32)) + 32;
    ...
    public static string DoubleToString(double d, int digitsLimit = _ToStringDigitsLimit, bool trailingZeros = false)
    {
        ...
        {
            var abs = Math.Abs(d);
            var neg = d < 0.0 || (d == -0.0 && IsNegativeZero(d)) ? 1 : 0;
    ...
    private static string lowLevelFormat(int digitsLimit, bool trailingZeros, double abs)
    {
        ...
        var m = ((ulong)raw & ((1UL << 52) - 1)) | highestBit;
        var e = (int)((raw & long.MaxValue) >> 52);
        e = 52 - e + 1023;
        ...
        if (fracPart != 0)
        {
            ...
            if (alterLen < fracLen) fracBuffer = alterFracBuffer;
        }
    ...
    private static void numStrMul10(ulong[] x, ref ulong[] output)
    {
        ...
        if (size > output.Length) output = new ulong[size];
