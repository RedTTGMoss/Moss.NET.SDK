...
public sealed class Promise
{
    ...
    private readonly TaskCompletionSource<JSValue> _outerTask;
    ...
    [Hidden] public Task<JSValue> Task => _outerTask.Task;

    [Hidden] public bool Complited => _outerTask.Task.IsCompleted;
    ...
    [Hidden] public Function Callback { get; }
    ...
    private void handlePromiseCascade(JSValue value, bool error)
    {
        ...
        if (task != null)
        {
            task.ContinueWith(t =>
            {
                if (t.IsFaulted)
                {
                    var exception = t.Exception.GetBaseException() as JSException ??
                                    t.Exception.GetBaseException() ?? t.Exception;
    ...
    private void callbackInvoke()
    {
        ...
        try
        {
            Callback.Call(new Arguments(null)
            {
    ...
    public static Promise race(IIterable promises)
    {
        if (promises == null)
            return new Promise(
                fromException(new JSException(new TypeError("Invalid argruments for Promise.race(...)"))));
    ...
    public static Promise all(IIterable promises)
    {
        if (promises == null)
            return new Promise(
                fromException(new JSException(new TypeError("Invalid argruments for Promise.all(...)"))));

        return new Promise(whenAll(promises.AsEnumerable().Select(convertToTask).ToArray())
            .ContinueWith(x => new Array(x.Result as IEnumerable) as JSValue));
    }
    ...
    {
        Func<JSValue, JSValue> func = onFinally == null
            ? null
            : value => onFinally.Call(JSValue.undefined, new Arguments { value });
    ...
    {
        var result = new JSValue[tasks.Length];
        var task = new TaskCompletionSource<JSValue[]>();
