...
{
    [ReadOnly] [DoNotDelete] [DoNotEnumerate] [NotConfigurable]
    public static readonly JSValue NaN = double.NaN;

    [ReadOnly] [DoNotDelete] [DoNotEnumerate] [NotConfigurable]
    public static readonly JSValue POSITIVE_INFINITY = double.PositiveInfinity;

    [ReadOnly] [DoNotDelete] [DoNotEnumerate] [NotConfigurable]
    public static readonly JSValue NEGATIVE_INFINITY = double.NegativeInfinity;

    [ReadOnly] [DoNotDelete] [DoNotEnumerate] [NotConfigurable]
    public static readonly JSValue MAX_VALUE = double.MaxValue;

    [ReadOnly] [DoNotDelete] [DoNotEnumerate] [NotConfigurable]
    public static readonly JSValue MIN_VALUE = double.Epsilon;

    [DoNotEnumerate]
    static Number()
    {
        POSITIVE_INFINITY._attributes |= JSValueAttributesInternal.DoNotDelete | JSValueAttributesInternal.ReadOnly |
                                         JSValueAttributesInternal.NonConfigurable |
                                         JSValueAttributesInternal.SystemObject;
        NEGATIVE_INFINITY._attributes |= JSValueAttributesInternal.DoNotDelete | JSValueAttributesInternal.ReadOnly |
                                         JSValueAttributesInternal.NonConfigurable |
                                         JSValueAttributesInternal.SystemObject;
        MAX_VALUE._attributes |= JSValueAttributesInternal.DoNotDelete | JSValueAttributesInternal.ReadOnly |
                                 JSValueAttributesInternal.NonConfigurable | JSValueAttributesInternal.SystemObject;
        MIN_VALUE._attributes |= JSValueAttributesInternal.DoNotDelete | JSValueAttributesInternal.ReadOnly |
                                 JSValueAttributesInternal.NonConfigurable | JSValueAttributesInternal.SystemObject;
        NaN._attributes |= JSValueAttributesInternal.DoNotDelete | JSValueAttributesInternal.ReadOnly |
                           JSValueAttributesInternal.NonConfigurable | JSValueAttributesInternal.SystemObject;
    }
    ...
    public static JSValue toPrecision(JSValue self, Arguments digits)
    {
        ...
        var d = Tools.JSObjectToDouble(self);
    ...
    public static JSValue toFixed(JSValue self, Arguments digits)
    {
        ...
        var dgts = Tools.JSObjectToInt32(digits[0], true);
        if (dgts < 0 || dgts > 20)
        ...
        if (dgts > 0)
            dgts++;
        return System.Math.Round(res, dgts)
            .ToString("0.00000000000000000000".Substring(0, dgts + 1), CultureInfo.InvariantCulture);
    }
    ...
    {
        return self._valueType == JSValueType.Integer
            ? self._iValue.ToString(CultureInfo.CurrentCulture)
            : self._dValue.ToString(CultureInfo.CurrentCulture);
    }
