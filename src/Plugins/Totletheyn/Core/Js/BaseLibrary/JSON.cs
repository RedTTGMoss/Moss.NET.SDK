...
public static class JSON
{
    ...
    public static JSValue parse(Arguments args)
    {
        ...
        var reviewer = length > 1 ? args[1]._oValue as Function : null;
                ...
                case ',':
                {
                    ...
                    if (frame.state == ParseState.Array)
                        frame = new StackFrame
                        {
                            ...
                            fieldName = (frame.valuesCount++).ToString(CultureInfo.InvariantCulture),
    ...
    private static string stringifyImpl(string key, JSValue obj, Function replacer, HashSet<string> keys, string space,
        HashSet<JSValue> processed, Arguments args)
    {
        if (replacer != null)
        {
            args[0] = "";
        ...
        try
        {
            ...
            if (obj._valueType < JSValueType.Object)
            {
                ...
                if ((obj.ValueType == JSValueType.Double && double.IsNaN(obj._dValue)) ||
                    double.IsInfinity(obj._dValue))
                    return "null";
            ...
            if (toJSONmemb._valueType == JSValueType.Function)
                return stringifyImpl("", (toJSONmemb._oValue as Function).Call(obj, null), null, null, space, processed,
                    null);

            if (obj._valueType >= JSValueType.Object &&
                !typeof(JSValue).GetTypeInfo().IsAssignableFrom(obj.Value.GetType().GetTypeInfo()))
            {
                ...
                if (currentContext != null)
                {
                    ...
                    if (serializer != null) return serializer.Serialize(key, value, replacer, keys, space, processed);
                }
            }
            ...
            foreach (var member in obj)
            {
                ...
                if (result[0] == '[')
                {
                    ...
                    if (int.TryParse(member.Key, out curentIndex))
                    {
                        ...
                        for (var i = curentIndex - 1; i-- > prevIndex;) result.Append("null,");
                    }
                }
                else
                {
