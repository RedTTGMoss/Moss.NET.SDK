...
public static class GlobalFunctions
{
    ...
    internal static JSValue parseInt(JSValue thisBind, Arguments args)
    {
        ...
        if (source._valueType == JSValueType.Double)
            return double.IsInfinity(source._dValue) || double.IsNaN(source._dValue) ? Number.NaN :
                source._dValue == 0.0 ? 0 : // +0 и -0 должны стать равными
    ...
    internal static JSValue parseFloat(JSValue thisBind, Arguments x)
    {
        ...
        if (source._valueType == JSValueType.Double)
            return source._dValue == 0.0
                ? (Number)0
                : // +0 и -0 должны стать равными
                source;
