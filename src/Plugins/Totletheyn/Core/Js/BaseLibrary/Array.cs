...
using PropertyPair = Totletheyn.Core.Js.Core.PropertyPair;
#if NET40
...
{
    private static readonly SparseArray<JSValue> emptyData = new();
    ...
    public Array(double length)
    {
        ...
        if (length > 0) _data[(int)((uint)length - 1)] = null;
    }
    ...
    public static JSValue copyWithin(JSValue self, Arguments args)
    {
        ...
        if (array != null)
        {
            for (long from = start + modifier, to = target + modifier;
                 count != 0;
                 from += direction, to += direction, count--) array._data[(int)to] = array._data[(int)from];
        }
        else
        {
            var key = new JSValue();
    ...
    public static JSValue fill(JSValue self, Arguments args)
    {
        ...
        if (array != null)
        {
            for (var i = start; i < end; i++) array._data[(int)i] = value.CloneImpl(false);
        }
        else
        {
            var key = new JSValue();
    ...
    private static void flatten(JSValue array, JSValue callbackFn, JSValue thisBind, int depth,
        Action<JSValue> pushItemCallback)
    {
    ...
    public static JSValue map(JSValue self, Arguments args)
    {
        ...
        var len = iterateImpl(self, args[0], args[1], undefined, undefined, false,
            (value, index, thisBind, jsCallback) =>
            {
                value = value.CloneImpl(false);
    ...
    {
        var arrayLike = args?[0] ?? undefined;
        ...
        if (args.Length == 1) simpleFunction = true;
        ...
        Func<JSValue, long, JSValue, ICallable, bool> callback = (value, index, thisBind, jsCallback) =>
        {
            ...
            if (simpleFunction)
                ...
                result[(int)index] = jsCallback.Call(thisBind, new Arguments { value, index, arrayLike })
                    .CloneImpl(false);
    ...
    public static JSValue includes(JSValue self, Arguments args)
    {
        ...
        iterateImpl(self, null, null, args?[1] ?? undefined, undefined, false, (value, index, thisBind, jsCallback) =>
        {
            if (args[0].IsNaN()
                    ? value.IsNaN()
                    : StrictEqual.Check(args[0], value))
            {
                result = index;
                return false;
            }
    ...
    private static long iterateImpl(JSValue self, JSValue callbackFn, JSValue thisBind, JSValue startIndexSrc,
        JSValue endIndexSrc, bool processMissing, Func<JSValue, long, JSValue, ICallable, bool> callback)
    {
        var arraySrc = self._oValue as Array;
        var nativeMode = arraySrc != null;
        ...
        if (length > 0)
        {
            if (!nativeMode)
            {
                ...
                {
                    for (var enumerator = source.GetEnumerator(false, EnumerationMode.RequireValues);
                         enumerator.MoveNext();)
                    {
    ...
    private static long reverseIterateImpl(JSValue self, Arguments args, JSValue startIndexSrc,
        Func<JSValue, long, JSValue, Function, bool> callback)
    {
        var arraySrc = self.Value as Array;
        var nativeMode = arraySrc != null;
        if (!self.Defined || (self._valueType >= JSValueType.Object && self._oValue == null))
        {
#if (PORTABLE || NETCORE)
...
#else
            var stackTrace = new StackTrace();
            ExceptionHelper.Throw(new TypeError("Cannot call Array.prototype." +
                                                stackTrace.GetFrame(stackTrace.FrameCount - 2).GetMethod().Name +
        ...
        else
        {
            ...
            while (moved)
            {
                ...
                while (moved && ((uint)mainEnum.Current.Key > startIndex || mainEnum.Current.Value == null ||
                                 !mainEnum.Current.Value.Exists))
                    moved = mainEnum.MoveNext();
    ...
    private static JSValue joinImpl(JSValue self, string separator, bool locale)
    {
        ...
        var sb = new StringBuilder((int)((_data.Length - 1) * separator.Length));
        JSValue t, temp = 0;
    ...
    public static JSValue pop(JSValue self)
    {
        ...
        if (selfa != null)
        {
            ...
            var newLen = (int)(selfa._data.Length - 1);
        ...
        if (selfa != null)
        {
            for (var i = selfa._data.Length >> 1; i-- > 0;)
            {
                ...
                if (item0._valueType == JSValueType.Property)
                    value0 = ((item0._oValue as PropertyPair).getter ?? Function.Empty).Call(self, null)
                        .CloneImpl(false);
                else
                    value0 = item0;
                ...
                if (item1._valueType == JSValueType.Property)
                    value1 = ((item1._oValue as PropertyPair).getter ?? Function.Empty).Call(self, null)
                        .CloneImpl(false);
                else
                    value1 = item1;
    ...
    public static JSValue reduce(JSValue self, Arguments args)
    {
        ...
        var len = (skip ? 0 : 1) + iterateImpl(self, args[0], null, undefined, undefined, false,
            (value, index, thisBind, jsCallback) =>
            {
                value = value.CloneImpl(false);
    ...
    private static JSValue spliceImpl(JSValue self, Arguments args, bool needResult, out long initialLength)
    {
        ...
        if (selfa != null)
        {
            ...
            var pos0 = (long)System.Math.Min(Tools.JSObjectToDouble(args[0]),
                length); // double потому, что нужно "с заполнением", а не "с переполнением"
            long pos1;
            if (args.Length > 1)
            {
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        if (t != null && t._valueType == JSValueType.Property)
                            ((t._oValue as PropertyPair).setter ?? Function.Empty).Call(self,
                                new Arguments { value.CloneImpl(false) });
                        else
                            t = value;
                    }
                } while (key != ownKey);
        ...
        else
        {
            ...
            else if (delta < 0)
            {
                ...
                for (var i = pos1; i < length; i++)
                {
                    ...
                    if (i >= length + delta) self.DeleteProperty(tjo);
            ...
            do
            {
                ...
                switch (key._valueType)
                {
                    ...
                    case JSValueType.String:
                    {
                        ...
                        {
                            var si = 0;
                            if (Tools.ParseJsNumber(skey, ref si, out var dindex)
                                && si == skey.Length
                                && dindex >= 0
            ...
            if (isIndex)
            {
                ...
                else
                {
                    ...
                    if (res is null || res._valueType < JSValueType.Undefined)
                    {
                        notExists._valueType = JSValueType.NotExistsInObject;
                        res = notExists;
                        if (memberScope != PropertyScope.Own) return __proto__.GetProperty(key, false, memberScope);
                    }
    ...
    private sealed class JSComparer : IComparer<JSValue>
    {
        ...
        private readonly Function comparer;
        private readonly JSValue first;
        private readonly JSValue second;
