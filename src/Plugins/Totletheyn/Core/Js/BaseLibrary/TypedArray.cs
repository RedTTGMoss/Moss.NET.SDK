...
public abstract class TypedArray : JSObject
{
    ...
    protected TypedArray(JSValue iterablyObject)
    {
        ...
        _valueType = JSValueType.Object;
        _oValue = this;
        foreach (var item in src._data.ReverseOrder)
            this[item.Key] = item.Value;
    }

    protected abstract JSValue this[int index] { get; set; }

    [Field] [DoNotEnumerate] public ArrayBuffer buffer { [Hidden] get; private set; }

    [Field] [DoNotEnumerate] public int byteLength { [Hidden] get; private set; }

    [Field] [DoNotEnumerate] public int byteOffset { [Hidden] get; private set; }

    [Field] [DoNotEnumerate] public Number length { [Hidden] get; private set; }

    [Field] [DoNotEnumerate] public abstract int BYTES_PER_ELEMENT { [Hidden] get; }

    [Hidden] public abstract Type ElementType { [Hidden] get; }
    ...
    protected internal sealed override JSValue GetProperty(JSValue key, bool forWrite, PropertyScope memberScope)
    {
        if (memberScope < PropertyScope.Super && key._valueType != JSValueType.Symbol)
        {
            ...
            switch (tname._valueType)
            {
                ...
                {
                    isIndex = tname._dValue >= 0 && tname._dValue < uint.MaxValue &&
