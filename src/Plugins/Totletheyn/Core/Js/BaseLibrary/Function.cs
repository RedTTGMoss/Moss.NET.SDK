...
{
    private static readonly FunctionDefinition creatorDummy = new();
    internal static readonly Function Empty = new();
    ...
    internal static readonly JSValue propertiesDummySM = new()
    {
        _valueType = JSValueType.Property,
        _oValue = new PropertyPair { getter = TTEProxy, setter = TTEProxy },
        _attributes = JSValueAttributesInternal.DoNotDelete | JSValueAttributesInternal.Immutable |
                      JSValueAttributesInternal.DoNotEnumerate | JSValueAttributesInternal.ReadOnly |
                      JSValueAttributesInternal.NonConfigurable
    };

    internal readonly FunctionDefinition _functionDefinition;

    [Hidden] internal readonly Context _initialContext;

    private Dictionary<Type, Delegate> _delegateCache;

    [Hidden] internal Number _length;
    ...
    public Function(Arguments args)
    {
        ...
        if (_initialContext == Context.DefaultGlobalContext || _initialContext == null)
            throw new InvalidOperationException(
                "Special Functions constructor can be called from javascript code only");
        ...
        for (var i = 0; i < len; i++)
            argn += args[i] + (i + 1 < len ? "," : "");
        var code = "function (" + argn + "){" + Environment.NewLine + (len == -1 ? "undefined" : args[len]) +
                   Environment.NewLine + "}";
        var func = FunctionDefinition.Parse(
            new ParseInfo(code, null)
            {
    ...
    {
        [Hidden] get => _functionDefinition._name;
    }
    ...
    {
        [Hidden] get => _functionDefinition?._body._strict ?? true;
    }

    [Hidden]
    public virtual CodeBlock Body
    {
        [Hidden] get => _functionDefinition != null ? _functionDefinition._body : null;
    }
    ...
    {
        [Hidden] get => _functionDefinition._kind;
    }
    ...
    public JSValue Construct(Arguments arguments)
    {
        ...
        var targetObject = ConstructObject();
    ...
    protected static void ThrowTypeError()
    {
        ExceptionHelper.Throw(
            new TypeError("Properties \"caller\", \"callee\" and \"arguments\" may not be accessed in strict mode."));
    }
    ...
    protected internal virtual JSValue ConstructObject()
    {
        ...
        targetObject.__proto__ = prototype._valueType < JSValueType.Object
            ? Context.GlobalContext._globalPrototype
            : prototype._oValue as JSObject;

        return targetObject;
    }

    internal virtual JSValue InternalInvoke(JSValue targetObject, Expression[] arguments, Context initiator,
    ...
    protected internal virtual JSValue Invoke(bool construct, JSValue targetObject, Arguments arguments)
    {
        ...
        if (_functionDefinition.trace)
            Console.WriteLine("DEBUG: Run \"" + _functionDefinition.Reference.Name + "\"");
#endif
        JSValue result = null;
    ...
    internal JSValue evaluateBody(Context internalContext)
    {
        ...
        if (ai._valueType == JSValueType.Undefined) return undefined;
    ...
    internal void initCachedReference(Context internalContext)
    {
        ...
        if (descriptor != null && descriptor.name != null && descriptor.cacheContext != internalContext._parent)
        {
            if (descriptor.cacheContext != null)
            {
                ...
                variable._attributes = JSValueAttributesInternal.DoNotDelete |
                                       JSValueAttributesInternal.DoNotEnumerate | JSValueAttributesInternal.ReadOnly;
            }
    ...
    internal void initContext(JSValue targetObject, Arguments arguments, bool storeValuesInContext,
    ...
    internal void initParameters(Arguments args, bool storeVariablesIntoContext, Context internalContext)
    {
        ...
        var min = System.Math.Min(args._iValue,
            _functionDefinition._parameters.Length -
            (_functionDefinition._functionInfo.ContainsRestParameters ? 1 : 0));

        JSValue[] defaultValues = null;
        Array restArray = null;
        if (_functionDefinition._functionInfo.ContainsRestParameters) restArray = new Array();
        ...
        for (var i = min; i < args._iValue; i++)
        {
            ...
            if (restArray != null) restArray._data.Add(t);
        }
    ...
    internal void storeParameters()
    {
        if (_functionDefinition._parameters.Length != 0)
        {
            ...
            for (var i = 0; i < _functionDefinition._parameters.Length; i++)
                context._variables[_functionDefinition._parameters[i].Name] =
                    _functionDefinition._parameters[i].cacheRes;
        }
    }
    ...
    [Hidden] internal JSValue _prototype;
            ...
            if (_prototype == null)
            {
                ...
                else
                {
                    ...
                    res._attributes = JSValueAttributesInternal.DoNotEnumerate | JSValueAttributesInternal.DoNotDelete |
                                      JSValueAttributesInternal.NonConfigurable;
                    (res._fields["constructor"] = CloneImpl(false))._attributes =
                        JSValueAttributesInternal.DoNotEnumerate;
                    _prototype = res;
                }
            }
