...
public sealed class Date
{
    ...
    public Date(Arguments args)
    {
        ...
        else
        {
            ...
            var y = Tools.JSObjectToInt64(args[0], 1, true);
            var m = Tools.JSObjectToInt64(args[1], 0, true);
            var d = Tools.JSObjectToInt64(args[2], 1, true);
            var h = Tools.JSObjectToInt64(args[3], 0, true);
            var n = Tools.JSObjectToInt64(args[4], 0, true);
            var s = Tools.JSObjectToInt64(args[5], 0, true);
            var ms = Tools.JSObjectToInt64(args[6], 0, true);
            if (y == long.MaxValue
                || y == long.MinValue)
            {
                _error = true;
    ...
    private void offsetTimeValue(JSValue value, long amort, long mul, bool correctTimeWithTimezone = true)
    {
        ...
        else
        {
            ...
            _timeZoneOffset = getTimeZoneOffset(_time);
            if (correctTimeWithTimezone) _time -= _timeZoneOffset - oldTzo;
        }
    }
    ...
    public JSValue getYear()
    {
        ...
        if (jsYear._valueType == JSValueType.Integer)
            jsYear._iValue -= 1900;
        else if (jsYear._valueType == JSValueType.Double) jsYear._dValue -= 1900;
    ...
    private int getMonthImpl(bool withTzo)
    {
        ...
        var y = t / _400yearsMilliseconds * 400;
        t %= _400yearsMilliseconds;
        y += System.Math.Min(3, t / _100yearsMilliseconds) *
             100; // 4 быть не должно, ведь мы уже проверили делимость на 400
        t -= System.Math.Min(3, t / _100yearsMilliseconds) * _100yearsMilliseconds;
        y += t / _4yearsMilliseconds * 4;
        t %= _4yearsMilliseconds;
        y += System.Math.Min(3, t / _yearMilliseconds) + 1;
        var isLeap = (y % 4 == 0 && y % 100 != 0) || y % 400 == 0 ? 1 : 0;
    ...
    public JSValue setTime(JSValue time)
    {
        ...
        {
            offsetTimeValue(time, _time - _unixTimeBase, 1, false);
        }
    ...
    public JSValue setMonth(JSValue month, JSValue day)
    {
        if (month != null && month.Exists)
        {
            ...
            {
                _time = _time - timeToMonthLengths[getMonthImpl(true)][isLeap(getYearImpl(true)) ? 1 : 0];
                _time = dateToMilliseconds(getYearImpl(true), intMonth, getDateImpl(true), getHoursImpl(true),
                    getMinutesImpl(true), getSecondsImpl(), getMillisecondsImpl());
            }
            else
            {
                _time = _time - timeToMonthLengths[getMonthImpl(true)][isLeap(getYearImpl(true)) ? 1 : 0] +
                        timeToMonthLengths[intMonth][isLeap(getYearImpl(true)) ? 1 : 0];
            }
        }
    ...
    {
        _time = dateToMilliseconds(Tools.JSObjectToInt64(year) + 1900, getMonthImpl(true), getDateImpl(true),
            getHoursImpl(true), getMinutesImpl(true), getSecondsImpl(), getMillisecondsImpl());
        return year;
    }
    ...
    {
        _time = dateToMilliseconds(Tools.JSObjectToInt64(year) + 1900, getMonthImpl(false), getDateImpl(false),
            getHoursImpl(false), getMinutesImpl(false), getSecondsImpl(), getMillisecondsImpl());
        return year;
    }
    ...
    public JSValue setFullYear(JSValue year, JSValue month, JSValue day)
    {
        if (year != null && year.Exists)
        {
            ...
            _time = dateToMilliseconds(Tools.JSObjectToInt64(year), getMonthImpl(false), getDateImpl(false),
                getHoursImpl(false), getMinutesImpl(false), getSecondsImpl(), getMillisecondsImpl());
            _error = _time < 5992660800000;
        }
    ...
    private JSValue toIsoString()
    {
        ...
        return getYearImpl(false).ToString("0000") +
               "-" + (getMonthImpl(false) + 1).ToString("00") +
    ...
    private string stringifyTime(bool withTzo, bool rfc1123)
    {
        ...
        var timeName =
            Context.CurrentGlobalContext.CurrentTimeZone.IsDaylightSavingTime(new DateTimeOffset(_time * _timeAccuracy,
                offset))
                ? Context.CurrentGlobalContext.CurrentTimeZone.DaylightName
                : Context.CurrentGlobalContext.CurrentTimeZone.StandardName;
        var res =
    ...
    private static bool parseSelf(string timeStr, out long time, out long timeZoneOffset)
    {
        ...
        try
        {
            ...
            timeZoneOffset = Context.CurrentGlobalContext.CurrentTimeZone
                .GetUtcOffset(new DateTime(time * _timeAccuracy)).Ticks / 10000;
    ...
    private static bool parseDateTime(string timeString, out long time, out long tzo)
    {
        try
        {
            ...
            if (dateTime.Kind == DateTimeKind.Local) time += tzo;
            return true;
        }
    ...
    private static DateTime getNow()
    {
        return TimeZoneInfo.ConvertTime(DateTime.UtcNow, TimeZoneInfo.Utc,
            Context.CurrentGlobalContext.CurrentTimeZone);
    }
