...
public sealed class Power : Expression
{
    ...
    {
        _tempContainer._dValue = Math.Pow(Tools.JSObjectToDouble(_left.Evaluate(context)),
            Tools.JSObjectToDouble(_right.Evaluate(context)));
        ...
        if (!res && _this == this)
            try
            {
                ...
                if (_right.ContextIndependent)
                {
                    if (_left.ContextIndependent)
                    {
                        _this = new Constant(Math.Pow(Tools.JSObjectToDouble(_left.Evaluate(null)),
                            Tools.JSObjectToDouble(_right.Evaluate(null))));
                    }
                    else
                    {
