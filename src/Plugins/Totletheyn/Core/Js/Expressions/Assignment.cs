...
public class Assignment : Expression
{
    ...
    public override JSValue Evaluate(Context context)
    {
        ...
        if (field._valueType == JSValueType.Property) return setProperty(context, source, field, temp);
        ...
        Options opts, FunctionInfo stats)
    {
        ...
        if (vr != null)
        {
            if (vr._descriptor.IsDefined)
            {
                ...
                else if (vr._descriptor.lastPredictedType != stype)
                {
                    ...
                    {
                        if (message != null && stype >= PredictedType.Undefined &&
                            vr._descriptor.lastPredictedType >= PredictedType.Undefined)
                            message(MessageLevel.Warning, Position, Length,
                                "Variable \"" + vr.Name +
                                "\" has ambiguous type. It can be make impossible some optimizations and cause errors.");
            ...
            {
                message(MessageLevel.CriticalWarning, Position, Length,
                    "Assign to undefined variable \"" + vr.Name + "\". It will declare a global variable.");
            }
        }
            ...
            && (_codeContext & CodeContext.InLoop) == 0)
        {
            ...
            for (var i = 0; last && i < variable._descriptor.references.Count; i++)
                last &= variable._descriptor.references[i].Eliminated ||
                        variable._descriptor.references[i].Position <= Position;
    ...
    {
        var f = _left.ToString();
        if (f[0] == '(')
            f = f.Substring(1, f.Length - 2);
        var t = _right.ToString();
        if (t[0] == '(')
