...
public sealed class Conditional : Expression
{
    ...
    public IList<Expression> Threads => new ReadOnlyCollection<Expression>(threads);
    ...
    {
        Parser.Build(ref _left, expressionDepth + 1, variables,
            codeContext | CodeContext.Conditional | CodeContext.InExpression, message, stats, opts);
        Parser.Build(ref threads[0], expressionDepth, variables,
            codeContext | CodeContext.Conditional | CodeContext.InExpression, message, stats, opts);
        Parser.Build(ref threads[1], expressionDepth, variables,
            codeContext | CodeContext.Conditional | CodeContext.InExpression, message, stats, opts);
