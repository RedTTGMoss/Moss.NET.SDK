...
public sealed class ExpressionTree : Expression
{
    ...
    private Expression getFastImpl()
    {
        switch (Type)
        {
            ...
            {
                return new Increment(_left ?? _right,
                    _left == null ? IncrimentType.Postincriment : IncrimentType.Preincriment);
            }
            ...
            {
                return new Decrement(_left ?? _right,
                    _left == null ? DecrimentType.Postdecriment : DecrimentType.Postdecriment);
            }
            ...
            case OperationType.Conditional:
            {
                while (_right is ExpressionTree
                       && (_right as ExpressionTree).Type == OperationType.None
    ...
    private static Expression deicstra(ExpressionTree statement)
    {
        ...
        var cur = statement._right as ExpressionTree;
        if (cur == null)
            return statement;

        var expStack = new Stack<Expression>();
        var types = new Stack<Expression>();
        ...
        while (cur != null)
        {
            ...
            for (; types.Count > 0;)
            {
                var topType = (int)(types.Peek() as ExpressionTree).Type;
                if ((topType & (int)OperationTypeGroups.Special) > ((int)cur.Type & (int)OperationTypeGroups.Special)
                    || ((topType & (int)OperationTypeGroups.Special) ==
                        ((int)cur.Type & (int)OperationTypeGroups.Special)
                        && ((int)cur.Type & (int)OperationTypeGroups.Special) > (int)OperationTypeGroups.Choice
                        && cur.Type != OperationType.Power))
                {
                    var expr = types.Pop();
                    expr._right = expStack.Pop();
                    expr._left = expStack.Pop();
                    expr.Position = (expr._left ?? expr).Position;
                    expr.Length = (expr._right ?? expr._left ?? expr).Length +
                        (expr._right ?? expr._left ?? expr).Position - expr.Position;
                    expStack.Push(expr);
                }
                else
                {
        ...
        while (expStack.Count > 1)
        {
            ...
            expr._left = expStack.Pop();
            expr.Position = (expr._left ?? expr).Position;
            expr.Length = (expr._right ?? expr._left ?? expr).Length + (expr._right ?? expr._left ?? expr).Position -
                          expr.Position;
            expStack.Push(expr);
        }
    ...
    {
        return Parse(state, ref index, false, true);
    }

    public static Expression Parse(ParseInfo state, ref int index, bool forUnaryOperator)
    {
        return Parse(state, ref index, forUnaryOperator, true);
    }

    internal static Expression Parse(ParseInfo state, ref int index, bool forUnary = false, bool processComma = true,
        bool forNew = false, bool root = true, bool forForLoop = false)
    {
        var i = index;
    ...
    private static Expression parseContinuation(ParseInfo state, Expression first, int startIndex, ref int i,
        ref bool root, bool forUnary, bool processComma, bool forNew, bool forForLoop)
    {
        Expression second = null;
        var kind = OperationType.None;
        var canAsign = !forUnary; // на случай f() = x
        var assign = false; // на случай операторов 'x='
        var binary = false;
            ...
            switch (state.Code[i])
            {
                        ...
                        if (state.Strict)
                            if (first is Variable
                                ...
                                ExceptionHelper.ThrowSyntaxError(
                                    "Cannot incriment \"" + (first as Variable).Name + "\" in strict mode.", state.Code,
                                    i);
                        first = new Increment(first, IncrimentType.Postincriment)
                            { Position = first.Position, Length = i + 2 - first.Position };
                        repeat = true;
                        i += 2;
                    }
                    else if (forUnary)
                ...
                case '-':
                {
                    if (state.Code[i + 1] == '-')
                    {
                        ...
                        if (state.Strict)
                            if (first is Variable
                                && ((first as Variable).Name == "arguments" || (first as Variable).Name == "eval"))
                                ExceptionHelper.Throw(new SyntaxError("Cannot decriment \"" + (first as Variable).Name +
                                                                      "\" in strict mode."));
                        first = new Decrement(first, DecrimentType.Postdecriment)
                            { Position = first.Position, Length = i + 2 - first.Position };
                        repeat = true;
                        i += 2;
                    }
                ...
                case '.':
                {
                    ...
                    if (!Parser.ValidateName(state.Code, ref i, false, true, state.Strict))
                        ExceptionHelper.Throw(new SyntaxError(string.Format(Strings.InvalidPropertyName,
                            CodeCoordinates.FromTextPosition(state.Code, i, 0))));

                    var name = state.Code.Substring(s, i - s);
                ...
                case '[':
                {
                    ...
                    var startPos = i;
                    var mname = Parse(state, ref i, false, true);

                    if (mname == null)
                        ExceptionHelper.Throw(new SyntaxError("Unexpected token at " +
                                                              CodeCoordinates.FromTextPosition(state.Code, startPos,
                                                                  0)));
                    ...
                    if (state.Message != null)
                    {
                        ...
                        if (cname != null
                            ...
                            state.Message(MessageLevel.Recomendation, mname.Position, mname.Length,
                                "[\"" + cname.value._oValue + "\"] is better written in dot notation.");
                    }
                ...
                case '(':
                {
                    ...
                    i++;
                    var startPos = i;
                    var withSpread = false;
                    for (;;)
                    {
                        Tools.SkipSpaces(state.Code, ref i);
                        Tools.CheckEndOfInput(state.Code, ref i);

                        var commaExists = args.Count == 0;
                        for (;;)
                            if (state.Code[i] == ',')
                            {
                                if (commaExists)
                                    ExceptionHelper.ThrowSyntaxError("Missing argument of function call", state.Code,
                                        i);
        ...
        if (state.Strict
            ...
            if (assign || kind == OperationType.Assignment)
                ExceptionHelper.ThrowSyntaxError("Assignment to eval or arguments is not allowed in strict mode",
                    state.Code, i);
        ...
        if (kind == OperationType.Assignment || assign)
        {
            ...
            if (error)
                ExceptionHelper.ThrowReferenceError(Strings.InvalidLefthandSideInAssignment, state.Code, first.Position,
                    first.Length);
        }
        ...
        if (binary && !forUnary)
        {
            ...
            if (state.Code.Length > i)
            {
                ...
                {
                    var proot = false;
                    second = parseContinuation(state, parseTernaryBranches(state, forForLoop, ref i), startIndex, ref i,
                        ref proot, forUnary, processComma, false, forForLoop);
                }
                else
                {
                ...
                else
                {
                    if (kind == OperationType.NullishCoalescing)
                        if (second is ExpressionTree tree
                            && (tree.Type == OperationType.LogicalAnd || tree.Type == OperationType.LogicalOr))
                            ExceptionHelper.ThrowSyntaxError(Strings.LogicalNullishCoalescing, state.Code, startIndex,
                                i - startIndex);

                    if (kind == OperationType.LogicalAnd || kind == OperationType.LogicalOr)
                        if (second is ExpressionTree tree
                            && tree.Type == OperationType.NullishCoalescing)
                            ExceptionHelper.ThrowSyntaxError(Strings.LogicalNullishCoalescing, state.Code, startIndex,
                                i - startIndex);

                    res = new ExpressionTree
                        { _left = first, _right = second, Type = kind, Position = startIndex, Length = i - startIndex };
                }
            }
            else
            {
        ...
        if (operand == null)
        {
            ...
            else if (Parser.ValidateValue(state.Code, ref i))
            {
                ...
                else
                {
                    ...
                    {
                        operand = new Constant(state.Code[start] == 't' ? Boolean.True : Boolean.False)
                    ...
                    else
                    {
                        var temp = start;
                        double d = 0;
                        if (Tools.ParseJsNumber(state.Code, ref temp, out d, 0,
                                ParseNumberOptions.Default |
                                (state.Strict ? ParseNumberOptions.RaiseIfOctal : ParseNumberOptions.None)))
                        {
                            ...
                            else
                            {
                                if (state.DoubleConstants.ContainsKey(d))
                                    operand = new Constant(state.DoubleConstants[d])
                                        { Position = start, Length = i - start };
                                else
                                    operand = new Constant(state.DoubleConstants[d] = d)
                                        { Position = start, Length = i - start };
                            }
                        }
                        ...
                        {
                            throw new ArgumentException("Unable to process value (" +
                                                        state.Code.Substring(start, i - start) + ")");
                        }
                    }
                }
            }
                    ...
                    case '+':
                    {
                        ...
                        if (state.Code[i] == '+')
                        {
                            ...
                            if (state.Strict
                                ...
                                ExceptionHelper.ThrowSyntaxError(
                                    "Cannot incriment \"" + (operand as Variable).Name + "\" in strict mode.",
                                    state.Code, i);
                    ...
                    case '-':
                    {
                        ...
                        if (state.Code[i] == '-')
                        {
                            ...
                            if (state.Strict
                                && operand is Variable
                                && ((operand as Variable).Name == "arguments" || (operand as Variable).Name == "eval"))
                                ExceptionHelper.Throw(new SyntaxError("Cannot decriment \"" +
                                                                      (operand as Variable).Name +
                                                                      "\" in strict mode."));
    ...
    private static Expression parseTernaryBranches(ParseInfo state, bool forEnumeration, ref int i)
    {
        ...
        try
        {
            ...
            result = new Constant(new JSValue { _valueType = JSValueType.Object, _oValue = threads })
                { Position = position };
    ...
    {
        return Type + "(" + _left + (_right != null ? ", " + _right : "") + ")";
    }
}