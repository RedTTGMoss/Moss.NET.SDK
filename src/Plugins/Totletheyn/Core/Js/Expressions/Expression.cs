...
public abstract class Expression : CodeNode
{
    ...
    internal void Optimize(ref Expression self, FunctionDefinition owner, InternalCompilerMessageCallback message,
    ...
    internal void baseOptimize(ref CodeNode _this, FunctionDefinition owner, InternalCompilerMessageCallback message,
        Options opts, FunctionInfo stats)
    {
    ...
    public sealed override void Decompose(ref CodeNode self)
    {
        if (NeedDecompose)
        {
            ...
            if (result.Count > 0) self = new SuspendableExpression(s, result.ToArray());
        }
    }

    public virtual void Decompose(ref Expression self, IList<CodeNode> result)
    {
        if (_left != null) _left.Decompose(ref _left, result);
