...
public sealed class Property : Expression
{
    ...
    private JSValue getPropertyGeneric(Context context, JSValue source)
    {
        ...
        else
        {
            if (res._valueType == JSValueType.NotExists)
                res._valueType = JSValueType.NotExistsInObject;
            else if (res._valueType == JSValueType.Property) res = Tools.GetPropertyOrValue(res, source);
        }
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        if (_left is Super)
            _memberScope = (codeContext & CodeContext.InStaticMember) != 0
                ? PropertyScope.Super
                : PropertyScope.PrototypeOfSuperClass;
