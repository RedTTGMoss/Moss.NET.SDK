...
public sealed class GetArgumentsExpression : Variable
{
    ...
    protected internal override JSValue EvaluateForWrite(Context context)
    {
        if (context._owner._functionDefinition._kind == FunctionKind.Arrow)
...
public class Variable : VariableReference
{
        ...
        List<CodeNode> dynamicValues)
    {
        ...
        var res = System.Linq.Expressions.Expression.Call(
            System.Linq.Expressions.Expression.ArrayAccess(JITHelpers.DynamicValuesParameter,
                JITHelpers.cnst(dynamicValues.Count - 1)),
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        if (!variables.TryGetValue(_variableName, out var desc) || desc == null)
        {
        ...
        if (expressionDepth >= 0 && expressionDepth < 2 && desc.IsDefined && !desc.lexicalScope &&
            (opts & Options.SuppressUselessExpressionsElimination) == 0)
        {
            ...
            if (message != null)
                message(MessageLevel.Warning, Position, Length,
                    "Unused getting of defined variable was removed. Maybe something missing.");
        }
            ...
            && (_descriptor.owner != owner || !owner._functionInfo.ContainsArguments))
        {
            ...
            if (assigns != null && assigns.Count > 0)
            {
                ...
                for (var i = assigns.Count; i-- > 0;)
                {
                    ...
                    {
                        if ((_codeContext & CodeContext.InLoop) != 0 &&
                            (assigns[i]._codeContext & CodeContext.InLoop) != 0)
                        {
                            lastAssign = null;
                            break;
                        }
                    ...
                    if (lastAssign == null || assigns[i].Position > lastAssign.Position) lastAssign = assigns[i];
                }
