...
public sealed class ClassDefinition : EntityDefinition
{
    ...
    private ClassDefinition(string name, Expression baseType, MemberDescriptor[] fields, FunctionDefinition ctor,
    ...
    public IEnumerable<MemberDescriptor> ComputedProperties => _computedProperties;
    ...
    internal static CodeNode Parse(ParseInfo state, ref int index)
    {
        ...
        while (state.Code[i] != '}')
            using (state.WithCodeContext(CodeContext.Strict | CodeContext.InExpression))
            {
                ...
                var @static = Parser.Validate(state.Code, "static", ref i);
                if (@static)
                {
                    Tools.SkipSpaces(state.Code, ref i);
                    s = i;
                ...
                var async = Parser.Validate(state.Code, "async", ref i);
                if (async)
                {
                    Tools.SkipSpaces(state.Code, ref i);
                    s = i;
                ...
                var getOrSet = Parser.Validate(state.Code, "get", ref i)
                ...
                if (Parser.Validate(state.Code, "[", ref i))
                {
                    ...
                    var propertyName = ExpressionTree.Parse(state, ref i, false, false);
                    ...
                    if (state.Code[i] == '(')
                        initializer = FunctionDefinition.Parse(state, ref i,
                            asterisk ? FunctionKind.AnonymousGenerator : FunctionKind.AnonymousFunction);
                    else
                    ...
                    switch (state.Code[s])
                    {
                        case 'g':
                        {
                            computedProperties.Add(new MemberDescriptor(propertyName,
                                new PropertyPair((Expression)initializer, null), @static));
                            break;
                        }
                        case 's':
                        {
                            computedProperties.Add(new MemberDescriptor(propertyName,
                                new PropertyPair(null, (Expression)initializer), @static));
                            break;
                        }
                        default:
                        {
                            computedProperties.Add(new MemberDescriptor(propertyName, (Expression)initializer,
                                @static));
                            break;
                        }
                    }
                ...
                else if (getOrSet)
                {
                    ...
                    if (!flds.ContainsKey(accessorName))
                    {
                        ...
                        flds.Add(accessorName,
                            new MemberDescriptor(new Constant(propertyAccessor._name), propertyPair, @static));
                    }
                    else
                    {
                        ...
                        if (vle == null)
                            ExceptionHelper.Throw(new SyntaxError("Try to define " +
                                                                  mode.ToString().ToLowerInvariant() +
                                                                  " for defined field at " +
                                                                  CodeCoordinates.FromTextPosition(state.Code, s, 0)));
                        ...
                        do
                        {
                            ...
                            ExceptionHelper.ThrowSyntaxError(
                                "Try to redefine " + mode.ToString().ToLowerInvariant() + " of " +
                                propertyAccessor.Name, state.Code, s);
                        } while (false);
                    }
                }
                else
                {
                    ...
                    if (fieldName == null)
                        ExceptionHelper.Throw(new SyntaxError("Invalid member name at " +
                                                              CodeCoordinates.FromTextPosition(state.Code, s, i - s)));

                    if (fieldName == "constructor")
                    {
                        if (@static) ExceptionHelper.ThrowSyntaxError(Strings.ConstructorCannotBeStatic, state.Code, s);
                        if (ctor != null)
                    ...
                    if (flds.ContainsKey(fieldName))
                        ExceptionHelper.Throw(new SyntaxError("Trying to redefinition member \"" + fieldName +
                                                              "\" at " + CodeCoordinates.FromTextPosition(state.Code, s,
                                                                  i - s)));
                    ...
                    if (async || asterisk || fieldName == "constructor" || state.Code[i] == '(')
                    {
                        i = s;
                        expression =
                            FunctionDefinition.Parse(state, ref i,
                                async ? FunctionKind.AsyncMethod : FunctionKind.Method) as FunctionDefinition;
                        if (expression == null)
        ...
        if (ctor == null)
        {
            ...
            {
                ctor = (FunctionDefinition)FunctionDefinition.Parse(nestedParseInfo, ref ctorIndex,
                    FunctionKind.Method);
            }
        }

        var classDefinition = new ClassDefinition(name, baseType, new List<MemberDescriptor>(flds.Values).ToArray(),
            ctor, computedProperties.ToArray());
        ...
        if ((state.CodeContext & CodeContext.InExpression) == 0)
            if ((state.CodeContext & CodeContext.InExport) == 0 || !string.IsNullOrEmpty(name))
            {
                ...
                if (state.Strict && state.FunctionScopeLevel != state.LexicalScopeLevel)
                    ExceptionHelper.ThrowSyntaxError(
                        "In strict mode code, class can only be declared at top level or immediately within other function.",
                        state.Code, index);
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        Parser.Build(ref _constructor, expressionDepth, variables,
            codeContext | CodeContext.InClassDefinition | CodeContext.InClassConstructor, message, stats, opts);
        ...
        {
            Parser.Build(ref _computedProperties[i]._name, 2, variables, codeContext | CodeContext.InExpression,
                message, stats, opts);
            Parser.Build(ref _computedProperties[i]._value, 2, variables, codeContext | CodeContext.InExpression,
                message, stats, opts);
        }

        if (descriptorToRestore != null)
            variables[descriptorToRestore.name] = descriptorToRestore;
        else if (!string.IsNullOrEmpty(_name)) variables.Remove(_name);
    ...
    public override JSValue Evaluate(Context context)
    {
        ...
        if ((_codeContext & CodeContext.InExpression) == 0 && !string.IsNullOrEmpty(_name))
            variable = context.DefineVariable(_name);
        ...
        if (_baseClass != null)
        {
            ...
            if (baseProto == null)
                ...
                ctor.prototype.__proto__ =
                    Tools.GetPropertyOrValue(baseProto.GetProperty("prototype"), baseProto)._oValue as JSObject;
    ...
    protected internal override CodeNode[] GetChildrenImpl()
    {
        ...
        for (var i = 0; i < _members.Length; i++) result.Add(_members[i]._value);
...
#if DEBUG
            Debug.Assert(result.Count == 0, "Decompose: results not empty");
#endif
        }
    ...
    private sealed class ClassConstructor : Function
    {
        ...
        public override string name => _classDefinition.Name;
