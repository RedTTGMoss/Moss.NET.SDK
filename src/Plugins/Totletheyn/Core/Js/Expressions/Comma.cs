...
public sealed class Comma : Expression
{
    ...
    protected internal override PredictedType ResultType => (_right ?? _left).ResultType;
        ...
        InternalCompilerMessageCallback message, FunctionInfo stats, Options opts)
    {
        ...
        Parser.Build(ref _left, expressionDepth + 1, variables, codeContext | CodeContext.InExpression, message, stats,
            opts);
        Parser.Build(ref _right, expressionDepth + 1, variables, codeContext | CodeContext.InExpression, message, stats,
            opts);
        return false;
    }
