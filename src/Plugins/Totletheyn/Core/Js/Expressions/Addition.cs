...
public sealed class Addition : Expression
{
        ...
        switch (first._valueType)
        {
            ...
            case JSValueType.Boolean:
            {
                ...
                switch (second._valueType)
                {
                    ...
                    case JSValueType.Boolean:
                    {
                        var tl = (long)first._iValue + second._iValue;
                        if ((int)tl == tl)
                        {
            ...
            case JSValueType.Double:
            {
                ...
                switch (second._valueType)
                {
                    ...
                    case JSValueType.String:
                    {
                        resultContainer._oValue =
                            new RopeString(NumberUtils.DoubleToString(first._dValue), second._oValue);
                        resultContainer._valueType = JSValueType.String;
                        return;
                    }
        ...
        Options opts, FunctionInfo stats)
    {
        ...
        if (ResultType is PredictedType.String) _this = new StringConcatenation([_left, _right]);
    }
#if !NETCORE
