...
public sealed class Script
{
    ...
    [ThreadStatic] private static Stack<Script> _scriptsStack;
    ...
    public static Script Parse(string code, CompilerMessageCallback messageCallback = null,
        Options options = Options.None)
    {
        ...
        var internalCallback = messageCallback != null
            ? (level, position, length, message) =>
                messageCallback(level, CodeCoordinates.FromTextPosition(code, position, length), message)
        ...
        var i = 0;
        var root = (CodeBlock)CodeBlock.Parse(
            new ParseInfo(code, internalCallback) { CodeContext = codeContext | CodeContext.AllowDirectives }, ref i);

        var stat = new FunctionInfo();
        Parser.Build(ref root, 0, new Dictionary<string, VariableDescriptor>(), codeContext, internalCallback, stat,
            options);
    ...
    public JSValue Evaluate(Context context)
    {
        ...
        lock (_scriptStackLock)
        {
            if (_scriptsStack == null)
                _scriptsStack = new Stack<Script>();
