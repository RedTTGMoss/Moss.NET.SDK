...
internal static class ExceptionHelper
{
    ...
    [ThreadStatic] private static JsStackFrame _executionStack;
    ...
    internal sealed class StackTraceState
    {
        ...
        public string ToString(JSException jSException)
        {
            ...
            var wordLine = originalStackTraceLines.FirstOrDefault(x => x.Contains(':'))?.Split(':')?.LastOrDefault()
                .Split(' ')[0] ?? "line";
            ...
            for (var i = 0; i < frames.Length; i++)
            {
                    ...
                    && method.GetCustomAttribute(typeof(StackFrameOverrideAttribute)) != null)
                {
                    ...
                    var codeCoords = code != null
                        ? CodeCoordinates.FromTextPosition(code, jsFrame.CodeNode?.Position ?? 0,
                            jsFrame.CodeNode?.Length ?? 0)
                        : null;
