...
internal class NcxWriter
{
    public static string Format(NcxDocument ncx)
    {
        ...
        foreach (var meta in ncx.Meta)
        {
            ...
            if (meta.Content != null) element.Add(new XAttribute(NcxMeta.Attributes.Content, meta.Content));
            if (!string.IsNullOrWhiteSpace(meta.Name)) element.Add(new XAttribute(NcxMeta.Attributes.Name, meta.Name));
    ...
    private static void WriteNavPoints(XElement root, IEnumerable<NcxNavPoint> navPoints)
    {
        foreach (var navPoint in navPoints)
        {
            ...
            if (navPoint.NavPoints.Any()) WriteNavPoints(element, navPoint.NavPoints);
        }
    }
}