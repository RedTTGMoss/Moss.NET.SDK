...
namespace Moss.NET.Sdk.Formats.Epub.Format.Writers;
...
internal class OpfWriter
{
    public static string Format(OpfDocument opf)
    {
        ...
        if (opf.Guide != null) root.Add(WriteGuide(opf.Guide));
    ...
    private static XElement WriteMetadata(OpfMetadata metadata, EpubVersion version)
    {
        ...
        foreach (var description in metadata.Descriptions) root.Add(new XElement(OpfElements.Description, description));
        foreach (var lang in metadata.Languages) root.Add(new XElement(OpfElements.Language, lang));
        foreach (var title in metadata.Titles) root.Add(new XElement(OpfElements.Title, title));
        ...
        foreach (var publisher in metadata.Publishers) root.Add(new XElement(OpfElements.Publisher, publisher));
        foreach (var subject in metadata.Subjects) root.Add(new XElement(OpfElements.Subject, subject));
        ...
        foreach (var coverage in metadata.Coverages) root.Add(new XElement(OpfElements.Coverages, coverage));
        ...
        foreach (var format in metadata.Formats) root.Add(new XElement(OpfElements.Format, format));
        ...
        foreach (var relation in metadata.Relations) root.Add(new XElement(OpfElements.Relation, relation));
        foreach (var right in metadata.Rights) root.Add(new XElement(OpfElements.Rights, right));
        foreach (var source in metadata.Sources) root.Add(new XElement(OpfElements.Source, source));
        foreach (var type in metadata.Types) root.Add(new XElement(OpfElements.Type, type));
    ...
    private static XElement WriteManifest(OpfManifest manifest)
    {
        ...
        foreach (var item in manifest.Items)
        {
            ...
            if (item.Fallback != null) element.Add(new XAttribute(OpfManifestItem.Attributes.Fallback, item.Fallback));
            if (item.FallbackStyle != null)
                element.Add(new XAttribute(OpfManifestItem.Attributes.FallbackStyle, item.FallbackStyle));
            if (item.Href != null) element.Add(new XAttribute(OpfManifestItem.Attributes.Href, item.Href));
            if (item.Id != null) element.Add(new XAttribute(OpfManifestItem.Attributes.Id, item.Id));
    ...
    private static XElement WriteSpine(OpfSpine spine)
    {
        ...
        if (spine.Toc != null) root.Add(new XAttribute(OpfSpine.Attributes.Toc, spine.Toc));

        foreach (var itemref in spine.ItemRefs)
        {
            var element = new XElement(OpfElements.ItemRef);
            if (itemref.Id != null) element.Add(new XAttribute(OpfSpineItemRef.Attributes.Id, itemref.Id));
            if (itemref.IdRef != null) element.Add(new XAttribute(OpfSpineItemRef.Attributes.IdRef, itemref.IdRef));
